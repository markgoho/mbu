...
async function purge(): Promise<PurgeResponse> {
  ...
  for (const doc of candidates.docs) {
    ...
    registrationsPurged += await purgeUniversity(doc.id);
  }

  return { universitiesProcessed, registrationsPurged };
}

async function purgeUniversity(universityId: string): Promise<number> {
...
export const RetentionServiceImpl: RetentionService = { purge };
