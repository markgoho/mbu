---
title: "Req 3b — Kayak Skills Check"
layout: "guide"
group_title: "Choose Your Craft"
req_number: "3b"
req_path: "3.b"
prev: "/merit-badges/whitewater/guide/req3a/"
prev_title: "Req 3a — Canoe Skills Check"
next: "/merit-badges/whitewater/guide/req4/"
next_title: "Req 4 — Pick Your Practice Track"
---

{{< drg/requirement number="3b" >}}
If you will be using a kayak to earn this merit badge, demonstrate strokes and maneuvers from the [Kayaking merit badge](/merit-badges/kayaking/) to the satisfaction of your counselor.
{{< /drg/requirement >}}

A whitewater kayak gives you more contact with the boat than an open canoe, but that does not make the basics optional. Your counselor wants to see that you already understand forward power, turning, edging, support strokes, and safe recovery habits before you add rapids to the picture.

Because kayaks respond quickly, sloppy technique also shows up quickly. If your torso rotation is weak, your strokes will be short and ineffective. If your edging is off, the hull may skid or catch in ways you did not expect. This requirement is your chance to prove that your kayaking foundation is ready for moving water.

{{< drg/checklist title="Kayak skills to review" subtitle="Prepare before your counselor watches" >}}
- **Forward stroke efficiency**: Use torso rotation, not just your arms.
- **Turning and steering**: Be able to place the bow and stern where you intend.
- **Support strokes**: Braces should feel like recovery tools, not desperate reactions.
- **Wet-exit confidence**: If you flip, you should know what to do without freezing.
{{< /drg/checklist >}}

{{< drg/tip >}}
If a stroke only works when the water is flat and calm, keep practicing. Whitewater demands strokes that still work when the boat is bouncing, angled, or crossing current.
{{< /drg/tip >}}

Once you choose your craft and confirm your base skills, you are ready for the calm-water practice track in [Req 4](/merit-badges/whitewater/guide/req4/).

{{< drg/next-page
    text="Your craft choice is set. Now choose the calm-water skill set that matches it."
    teaser="Compare the tandem canoe, solo canoe, and solo kayak practice paths before you start the detailed stroke work."
    url="/merit-badges/whitewater/guide/req4/" >}}
