...
req_number: "5b"
prev: "/merit-badges/chess/guide/req5a/"
prev_title: "Chess Strategy"
next: "/merit-badges/chess/guide/req5c/"
next_title: "Checkmate with Major Pieces"
---
...
Tactics are short, concrete sequences of moves that achieve a specific goal — winning material, delivering checkmate, or gaining a decisive advantage. While strategy is your long-term plan (as you learned in [Req 5a](/merit-badges/chess/guide/req5a/)), tactics are the *weapons* you use to execute that plan. Let's walk through each tactic on the list.
...
{{< drg/tip >}}
An easy way to remember the difference: in a **pin**, the less valuable piece is in front (shielding the more valuable one). In a **skewer**, the more valuable piece is in front (forced to move, exposing the one behind).
{{< /drg/tip >}}
...
{{< drg/did-you-know >}}
Double check is so powerful that even a pawn can participate. If a pawn captures a piece and reveals a discovered check from a bishop while the pawn itself gives check, the opponent's king must move — no matter what other pieces are available for defense.
{{< /drg/did-you-know >}}
...
**Interposing** means placing a piece between an attacker and its target (usually the king) to block the attack. This is one of the three ways to escape check (you learned this in [Req 3c](/merit-badges/chess/guide/req3c/)). As a tactic, an interposing move can sometimes block an attack while simultaneously creating a counter-threat.
...
**Zugzwang** (German for "compulsion to move") is a situation where every possible move makes the position worse. The player *would prefer to pass* but the rules require them to move. Zugzwang most commonly occurs in endgames and is a key weapon in king-and-pawn endings (which you will study in [Req 5d](/merit-badges/chess/guide/req5d/)).

{{< drg/be-prepared title="Choosing Your Five" >}}
You need to explain five of these fourteen tactics. For each one:

- **Define it** in one sentence.
- **Give an example** — describe a specific piece arrangement that illustrates the tactic.
- **Explain why it works** — what makes the opponent unable to deal with it?

Practice by solving tactical puzzles on Lichess or Chess.com. The more puzzles you solve, the more naturally you will spot these patterns in your own games.
{{< /drg/be-prepared >}}

{{< drg/image src="images/chess-tactics-fork-pin.avif" alt="Comparison diagram showing a knight fork on the left (knight attacking king and rook simultaneously) and a bishop pin on the right (bishop pinning a knight to the king)" >}}

{{< drg/external-link
    title="Lichess Tactics Trainer"
    url="https://lichess.org/training"
    description="Unlimited free tactical puzzles sorted by theme (forks, pins, skewers, discovered attacks, etc.). The best way to sharpen your tactical vision." >}}

Now let's put your knowledge to work on the board. Time to learn how to force checkmate with major pieces.

{{< drg/next-page
    text="Now that you know the tactical toolkit"
    teaser="Learn how to force checkmate with rooks and queen against a lone king."
