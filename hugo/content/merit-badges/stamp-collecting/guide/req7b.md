---
title: "Visit the Stamp World"
layout: "guide"
group_title: "7. Creative Projects"
req_number: "7b"
req_path: "7.b"
prev: "/merit-badges/stamp-collecting/guide/req7a/"
prev_title: "Design Work"
next: "/merit-badges/stamp-collecting/guide/req7c/"
next_title: "Review a Stamp Article"
---

{{< drg/requirement number="7b" >}}
Visit a post office, stamp club, or stamp show with an experienced collector. Explain what you saw and learned.
{{< /drg/requirement >}}

This option is about learning from real people in real places. A visit helps you see how stamp collecting works outside your own album — how material is bought, sold, displayed, discussed, and cared for.

### What to notice during your visit

At a post office, pay attention to current stamp issues, postal products, and how mailing services work. At a stamp club, notice how collectors organize discussions and share knowledge. At a stamp show, look at dealer tables, exhibit pages, judging standards, and the wide variety of specialties.

{{< drg/checklist title="Questions to bring on your visit" subtitle="These will help you explain what you learned later" >}}
- **What kinds of collections are people building?**
- **How are stamps stored and displayed?**
- **What tools do experienced collectors use most often?**
- **What surprised you about the hobby in person?**
{{< /drg/checklist >}}

{{< drg/video
    title="Visiting a Stamp Show (video)"
    url="https://youtu.be/5bImEro1G30?si=nz4q-uxDtxbHqgXZ" >}}

{{< drg/video
    title="The Great American Stamp Show 2024 Wrap Up! (video)"
    url="https://youtu.be/Gtpcz8eCdSU?si=qkd88rX16c3iYwPv" >}}

{{< drg/video
    title="The Great American Stamp Show 2025 (video)"
    url="https://youtu.be/867c5dwUzp0?si=F9Lz9rgw2RRY3Zv1" >}}

{{< drg/tip >}}
Take notes during the visit, not just afterward. A short list of dealer comments, exhibit ideas, and surprising observations will make your explanation much more specific.
{{< /drg/tip >}}

A visit reminds you that stamp collecting is not a lonely hobby. It is a community built around shared curiosity.

{{< drg/next-page
    text="You know how to turn a visit into a useful learning experience."
    teaser="Next, learn how to read and review a stamp article instead of just skimming it."
    url="/merit-badges/stamp-collecting/guide/req7c/" >}}
