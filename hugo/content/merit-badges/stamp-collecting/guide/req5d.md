---
title: "Req 5d — Hinges and Mounts"
layout: "guide"
group_title: "Collector's Tools"
req_number: "5d"
req_path: "5.d"
prev: "/merit-badges/stamp-collecting/guide/req5c/"
prev_title: "Req 5c — Inspecting With Magnifiers"
next: "/merit-badges/stamp-collecting/guide/req5e/"
next_title: "Req 5e — Measuring With a Perforation Gauge"
---

{{< drg/requirement number="5d" >}}
Hinges and stamp mounts
{{< /drg/requirement >}}

Hinges and mounts both attach stamps to album pages, but they do it in different ways. Knowing when to use each one shows that you understand both presentation and preservation.

### Hinges

A hinge is a small folded strip of gummed paper. One part sticks lightly to the back of the stamp, and the other part sticks to the album page. Hinges are inexpensive and work well for many used stamps.

### Mounts

A mount is a clear protective sleeve that holds the stamp without sticking directly to the front or back. Many collectors prefer mounts for mint stamps, valuable stamps, or stamps with delicate gum because mounts offer better protection.

### How to compare them

- **Cost**: Hinges are cheaper.
- **Protection**: Mounts protect better.
- **Best use**: Hinges often suit ordinary used stamps. Mounts often suit mint or higher-value material.
- **Appearance**: Mounts give a clean, framed display on album pages.

{{< drg/image src="images/hinge-vs-mount-comparison.avif" alt="Side-by-side comparison of the same stamp mounted with a stamp hinge on one album page and inside a clear stamp mount on another, showing attachment method and protection difference" >}}

{{< drg/video
    title="How to Trim Self-Adhesive Stamps for Mounts (video)"
    url="https://youtu.be/_HT-sU7NHdI?si=X14V2fIh0yn2ndZd" >}}

{{< drg/tip >}}
If you demonstrate both on common, inexpensive stamps, your counselor can see the difference without risking damage to something valuable.
{{< /drg/tip >}}

After this tool, you will move from handling and storage to measurement.

{{< drg/next-page
    text="You can now explain the difference between hinges and mounts."
    teaser="Next, learn how a perforation gauge helps identify stamps that look almost identical."
    url="/merit-badges/stamp-collecting/guide/req5e/" >}}
