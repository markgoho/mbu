---
title: "Swimmer Test Readiness"
layout: "guide"
group_title: "1. Safety, Readiness & Boat Setup"
req_number: "2"
req_path: "2"
prev: "/merit-badges/small-boat-sailing/guide/req1/"
prev_title: "Safety, Injuries & Safety Afloat"
next: "/merit-badges/small-boat-sailing/guide/req3/"
next_title: "Know Your Boat"
---

{{< drg/requirement number="2" >}}
Before doing the following requirements, successfully complete the Scouting America swimmer test. Note: See the Swimming merit badge pamphlet for details about the Scouting America swimmer test.
{{< /drg/requirement >}}

A sailor plans to stay in the boat, but smart sailors prepare for the moment something goes wrong. A capsize, a slip at the dock, or a rescue drill can put you in the water fast. That is why this requirement comes early. Your counselor needs to know that you can stay calm and function in the water before you begin the badge's hands-on sailing work.

## Why this matters in sailing

Small-boat sailing happens in an environment where wind, waves, and gear can change the situation quickly. If you end up overboard or beside a capsized boat, panic makes every decision worse. The swimmer test helps prove that you can move with control, manage your breathing, and finish a demanding task even when you are tired.

The test is also part of the bigger Safety Afloat system you studied in [Req 1](/merit-badges/small-boat-sailing/guide/req1/). Swimming ability is one of the nine points because boating safety depends on honest skill assessment.

{{< drg/checklist title="How to Prepare for the Swimmer Test" subtitle="Steady effort matters more than speed" >}}
- **Know the sequence:** Ask your counselor or aquatics staff to review the full test before you begin.
- **Use smooth strokes:** Splashing wastes energy.
- **Pace the early part:** Do not burn yourself out trying to impress anyone in the first few lengths.
- **Practice treading water when tired:** The finish still counts.
- **Be honest:** If you need more practice, say so and train for it.
{{< /drg/checklist >}}

## What your counselor wants to hear

A strong discussion of this requirement is not just, “I passed.” Explain why the swimmer test belongs in a sailing badge:

- it prepares you for capsize and rescue drills
- it supports buddy-based boating activities
- it lowers panic risk during a real emergency
- it proves you are ready for more advanced on-the-water skills

{{< drg/tip >}}
If you have not been swimming much lately, practice the full sequence before test day. Calm rhythm is better than trying to muscle your way through it.
{{< /drg/tip >}}

{{< drg/be-prepared title="You feel nervous right before the test" >}}
- **Listen carefully** so you know exactly what comes next.
- **Take the test one segment at a time** instead of thinking about the whole thing at once.
- **Settle into a repeatable stroke pattern** and breathe on purpose.
- **Tell the supervisor** if something feels wrong. Safety matters more than pride.
{{< /drg/be-prepared >}}

{{< drg/external-link
    title="Scouting America — Safety Afloat"
    url="https://www.scouting.org/health-and-safety/safety-afloat/"
    description="Shows how swimming ability fits into Scouting's full boating-safety policy." >}}

Passing the swimmer test clears you for the badge's sailing drills. Next, make sure you can name and understand the boat itself.

{{< drg/next-page
    text="You are proving you belong in the water environment, not just in the boat."
    teaser="Next, learn the major parts of your sailboat and what each one does."
    url="/merit-badges/small-boat-sailing/guide/req3/" >}}