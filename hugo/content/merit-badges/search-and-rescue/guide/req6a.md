---
title: "Req 6a — Search vs. Rescue"
layout: "guide"
group_title: "Search Procedure Skills"
req_number: "6a"
req_path: "6.a"
prev: "/merit-badges/search-and-rescue/guide/req6/"
prev_title: "Req 6 — Choose Four Search Procedure Skills"
next: "/merit-badges/search-and-rescue/guide/req6b/"
next_title: "Req 6b — PLS and LKP"
---

{{< drg/requirement number="6a" >}}
Explain the difference between search and rescue.
{{< /drg/requirement >}}

People often say "search and rescue" as if it were one job, but the two words describe different parts of the mission.

**Search** means locating the subject, their clues, or their most likely area. **Rescue** means helping that person survive, treating urgent problems, and getting them to safety.

A mission may start as a search and become a rescue the moment the subject is found alive and needs medical care, packaging, transport, or evacuation. It may also become a recovery operation if the subject is not alive.

## Why the distinction matters

If teams blur these ideas together, they can miss important planning needs.

- A search plan focuses on clues, probability, terrain, and where to deploy teams.
- A rescue plan focuses on patient condition, hazards, equipment, evacuation route, and medical support.

For example, finding an injured hiker in a canyon does not end the mission. It starts a new phase that may need litter teams, rope systems, medics, and a safer route out.

{{< drg/be-prepared title="Your team locates the subject quickly" >}}
That is good news, but the mission is not automatically over.

- **First:** Report the find accurately.
- **Next:** Assess the subject's condition and immediate hazards.
- **Then:** Shift from search mindset to rescue mindset if the person needs care or evacuation.
  {{< /drg/be-prepared >}}

{{< drg/tip >}}
A simple way to remember it: search answers "Where are they?" Rescue answers "How do we get them out safely?"
{{< /drg/tip >}}

{{< drg/external-link
    title="National Association for Search and Rescue"
    url="https://www.nasar.org/"
    description="Background on the range of search and rescue disciplines, from locating subjects to specialized rescue operations." >}}

Next, learn the two starting-point terms that often shape where a mission begins on the map.

{{< drg/next-page
    text="Move from mission phases to map starting points."
    teaser="Learn the difference between a place last seen and a last known point."
    url="/merit-badges/search-and-rescue/guide/req6b/" >}}
