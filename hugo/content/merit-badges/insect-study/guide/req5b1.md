---
title: "Inside an Ant Colony"
layout: guide
group_title: "5. Choosing a Colony Study"
req_number: "5b1"
prev: "/merit-badges/insect-study/guide/req5b/"
prev_title: "Pick Your Colony Project"
next: "/merit-badges/insect-study/guide/req5b2/"
next_title: "Reading a Bee Hive"
---

{{< drg/requirement number="5b1" >}}
1. Observe an ant colony in a formicarium (ant farm). Find the queen and worker ants. Explain to your counselor the different chambers found within an ant colony.
{{< /drg/requirement >}}

An ant formicarium lets you see something most people miss: the inside life of a colony. Instead of watching ants only as tiny dots on the ground, you can study how they divide space, care for young, and move food through the nest.

Start by finding the **queen**. She is often larger than the workers, with a thicker thorax or abdomen depending on the species. In some colonies she stays deeper in the nest, especially near brood. The **workers** are the ones you will usually see moving the most. They forage, carry food, clean chambers, and care for larvae and pupae.

Different chambers serve different purposes. Not every ant species builds the same layout, and not every formicarium will show every chamber clearly, but you can often explain several functional areas:

- **Brood chambers** where eggs, larvae, and pupae are kept
- **Food storage areas** where food is gathered or processed
- **Resting or queen areas** where the queen spends much of her time
- **Tunnels and travel routes** that connect the chambers and allow ants to move efficiently
- **Waste areas** where some species place dead ants or debris away from brood

{{< drg/image src="images/ant-colony-chambers-formicarium-diagram.avif" alt="Cutaway diagram of a formicarium with labeled brood chamber, food storage area, queen chamber, connecting tunnels, and waste area, plus visible queen and worker ants" >}}

The goal is not just to point and name. Watch how ants use the space. Are workers clustering around brood? Are they carrying food toward one part of the colony? Are there chambers used more often than others?

{{< drg/video
    title="Inside the Ant Colony ( video)"
    url="https://www.youtube.com/watch?v=vG-QZOTc5_Q" >}}

{{< drg/tip >}}
Spend several short sessions observing instead of one rushed look. Ant colonies often reveal more when you watch patterns over time, such as where workers bring food or how brood gets moved after disturbance.
{{< /drg/tip >}}

{{< drg/be-prepared title="If you cannot find the queen right away" >}}
Do not assume she is missing.

- **Watch the quieter chambers**: The queen often stays deeper in the nest.
- **Look for size differences**: She may be longer or broader than workers.
- **Be patient**: Colonies shift positions during feeding, cleaning, or changes in light.
- **Describe what you can prove**: Even if the queen stays hidden, you can still explain worker roles and chamber functions.
  {{< /drg/be-prepared >}}

This option pairs nicely with Req 5a because it gives you direct evidence of what social behavior looks like. The colony is not just a crowd. It is an organized system.

{{< drg/next-page
    text="You have explored how an ant colony is organized and what the chambers inside it are used for."
    teaser="For comparison, look at the bee-hive option and see how another social insect colony is structured."
    url="/merit-badges/insect-study/guide/req5b2/" >}}
