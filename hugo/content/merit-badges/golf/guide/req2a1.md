---
title: "Req 2a1 — Rules, Terms & Amateur Status"
layout: guide
group_title: "Traditional Golf Path"
req_number: "2a1"
prev: "/merit-badges/golf/guide/req2a/"
prev_title: "Req 2a — Traditional Golf Roadmap"
next: "/merit-badges/golf/guide/req2a2/"
next_title: "Req 2a2 — Understanding the Handicap System"
---

{{< drg/requirement number="2a1" >}}
Study the USGA Rules of Golf now in use.
{{< /drg/requirement >}}

This requirement is about more than memorizing rule numbers. It teaches you how golfers keep the game fair even when no referee is standing nearby. The three parts below fit together: etiquette tells you how to behave, terminology helps you discuss the game clearly, and amateur status explains how competition stays true to the spirit of the sport.

{{< drg/external-link
    title="USGA Rules of Golf (website)"
    url="https://www.usga.org/rules/rules-and-clarifications/rules-and-clarifications.html#!ruletype=fr&section=rule&rulenum=1"
    description="The official USGA rules site lets you browse the current rules and clarifications used in play." >}}

{{< drg/external-link
    title="USGA Rules of Golf (Free Course) (website)"
    url="https://rules.usga.org/courses/rules-101/"
    description="A beginner-friendly course that explains the rules in smaller lessons with practical examples." >}}

{{< drg/video
    title="The Golf Clubs in Your Bag (video)"
    url="https://youtu.be/jbCekejTJXA?si=bsNNU6FHDV0kRcmb" >}}

Knowing your clubs supports the rules because many golf terms are tied to how and where different clubs are used. As you study, match the words you hear to real situations on a course.

## Requirement 2a1a

{{< drg/requirement number="2a1a" >}}
Tell about the three categories of golf etiquette.
{{< /drg/requirement >}}

Golf etiquette is usually grouped into three big ideas: **safety**, **consideration for others**, and **care of the course**. Safety means you never hit until the players ahead are clearly out of range, you stay clear of another player's swing, and you warn others if a ball is heading toward them. Consideration means keeping noise down, being ready when it is your turn, and not distracting another player. Care of the course means repairing ball marks, replacing divots when required, raking bunkers, and leaving the course better than you found it.

{{< drg/external-link
    title="Three Categories of Golf Etiquette (PDF)"
    url="https://filestore.scouting.org/filestore/Merit_Badge_ReqandRes/Requirement%20Resources/Golf/three_categories_of_golf_etiquette.pdf"
    description="A short official reference that organizes golf etiquette into the main categories Scouts should know." >}}

A good way to explain these categories to your counselor is to connect each one to a real moment in play. For example, waiting until the group ahead is clear is safety, standing still while someone putts is consideration, and fixing a pitch mark on the green is care of the course.

## Requirement 2a1b

{{< drg/requirement number="2a1b" >}}
Demonstrate that you understand the definitions of golf terms.
{{< /drg/requirement >}}

Golf has its own vocabulary, and learning it makes the game far less confusing. Words like **par**, **birdie**, **bogey**, **tee box**, **fairway**, **rough**, **hazard**, **bunker**, **green**, **stroke**, and **penalty area** come up constantly. You do not need to sound like a TV announcer. You do need to understand what the terms mean so you can follow directions, talk with your counselor, and know what is happening during play.

{{< drg/video
    title="Basic Golf Terminology (video)"
    url="https://youtu.be/3qWh2Wruokg?si=8wh6SORdQITXot0t" >}}

{{< drg/tip >}}
Do not study golf terms as a random list. Group them by place on the course, scoring, equipment, and rules. When words are connected to a picture in your mind, they are easier to remember.
{{< /drg/tip >}}

{{< drg/image src="images/golf-course-terms-map.avif" alt="Labeled diagram of a golf hole showing the tee box, fairway, rough, bunker, green, and penalty area in their real positions" >}}

## Requirement 2a1c

{{< drg/requirement number="2a1c" >}}
Show that you understand the Rules of Amateur Status.
{{< /drg/requirement >}}

Amateur status is about keeping the game based on skill, competition, and integrity rather than turning every event into a money contest. In simple terms, amateur golfers may compete and win certain prizes under the rules, but there are limits on accepting prize money or acting like a professional player without changing status. You should understand that the rules protect fairness and keep amateur competition separate from professional golf.

{{< drg/video
    title="Rules of Amateur Status (video)"
    url="https://youtu.be/TeMdMFFOaTo?si=v2Fu90PKhD6_ixkf" >}}

If your counselor asks why amateur status matters, a strong answer is that it protects the purpose of amateur competition. Scouts, junior golfers, school teams, and everyday players can compete without the game being driven mainly by cash rewards.

{{< drg/did-you-know >}}
One of golf's most unusual traditions is that players are expected to call penalties on themselves. That honor-based system only works when golfers respect both etiquette and the rules.
{{< /drg/did-you-know >}}

Before moving on, make sure you can explain not just what the rules say, but why they matter. That will help a lot when you get to handicaps, playing a round, and talking with your counselor about how golf stays fair.

{{< drg/next-page
    text="With rules and terminology in place, you are ready to tackle one of golf's most useful fairness tools."
    teaser="Learn how the World Handicap System helps golfers of different skill levels compete fairly."
    url="/merit-badges/golf/guide/req2a2/" >}}
