---
title: "Req 5 — Choose Your Mining Experience"
layout: "guide"
group_title: "Choose Your Mining Experience"
req_number: "5"
req_path: "5"
prev: "/merit-badges/mining-in-society/guide/req4/"
prev_title: "Req 4 — Why Abandoned Mines Are Dangerous"
next: "/merit-badges/mining-in-society/guide/req5a/"
next_title: "Req 5a — Compare Two Virtual Mine Tours"
---

{{< drg/requirement number="5" >}}
Do ONE of the following:
{{< /drg/requirement >}}

You must choose exactly one option for this requirement. Each path teaches something different about mining, so the best choice depends on what access you have, how much travel is realistic, and whether you are most interested in history, equipment, processing, or seeing mining in action.

## Your Options

- **[Req 5a — Compare Two Virtual Mine Tours](/merit-badges/mining-in-society/guide/req5a/)**: Explore two mine types online and compare how they plan, permit, equip, and produce resources.
- **[Req 5b — Museum Mining History](/merit-badges/mining-in-society/guide/req5b/)**: Visit a museum exhibit and connect mining history to larger events in society.
- **[Req 5c — Visit an Active Mine](/merit-badges/mining-in-society/guide/req5c/)**: See a working operation and follow the path from exploration to processing.
- **[Req 5d — Mining Equipment in Action](/merit-badges/mining-in-society/guide/req5d/)**: Visit an equipment maker or supplier and learn how machines fit into the mining process.
- **[Req 5e — Ore Processing Basics](/merit-badges/mining-in-society/guide/req5e/)**: Focus on how rock is broken down and how minerals are separated or purified.
- **[Req 5f — Your Community's Mining Story](/merit-badges/mining-in-society/guide/req5f/)**: Research a local mine and how it shaped your area socially, culturally, and economically.

## How to choose

| Option | Best for Scouts who... | Equipment / travel needs | What you will gain |
|---|---|---|---|
| 5a | Need a flexible option from home | Internet access | Comparison skills and a broad look at different mine types |
| 5b | Like history and museums | Travel to an exhibit | A stronger understanding of mining's role in past events |
| 5c | Want the most direct real-world experience | Permission, travel, guided visit | Firsthand insight into how a working mine operates |
| 5d | Enjoy machinery and engineering | Permission, travel, guided visit | A clear view of how mining equipment supports each work stage |
| 5e | Prefer chemistry and processing systems | Mostly desk research | A stronger grasp of ore reduction, extraction, and metal production |
| 5f | Want local relevance and interviews | Research time, local contacts | A richer picture of how mining has shaped your own community |

{{< drg/checklist title="Questions to ask before choosing" subtitle="Pick the option you can complete well, not just the one that sounds coolest" >}}
- **What access do you have?** A mine visit is great, but a virtual tour may be the most practical.
- **How much time do you have?** A local history project can take longer if you need interviews or archives.
- **What interests you most?** History, field visits, engineering, and chemistry each point to a different option.
- **What evidence can you bring?** Notes, photos, brochures, screenshots, and comparisons all make your counselor discussion stronger.
{{< /drg/checklist >}}

{{< drg/tip >}}
If you already enjoyed mapping operations in [Req 2](/merit-badges/mining-in-society/guide/req2/) and thinking about how sites work, Req 5c or Req 5d may feel like a natural next step. If you liked the global and research side of Req 1, Req 5a, 5e, or 5f may fit better.
{{< /drg/tip >}}

Choose the path that gives you the clearest, most detailed conversation with your counselor. The first option is virtual and flexible, so that is the one linked next.

{{< drg/next-page
    text="You have compared the six ways to complete this requirement"
    teaser="Start with the virtual-tour option and see how to compare two different mine types in a smart, organized way."
    url="/merit-badges/mining-in-society/guide/req5a/" >}}
