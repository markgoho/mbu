---
title: "Req 4 — Braiding and Plaiting"
layout: guide
group_title: "Building Your Project"
req_number: "4"
prev: "/merit-badges/leatherwork/guide/req3h/"
prev_title: "Req 3h — Smooth Finished Edges"
next: "/merit-badges/leatherwork/guide/req5/"
next_title: "Req 5 — Pick Your Leather Path"
---

{{< drg/requirement number="4" >}}
Braid or plait an article out of leather, vinyl lace, or paracord.
{{< /drg/requirement >}}

Braiding feels different from cutting and stitching because the material stays in motion. Instead of shaping flat pieces, you are building a pattern strand by strand. The challenge is keeping tension even so the braid looks organized instead of lumpy.

{{< drg/video
    title="Four Strand Round Braid (video)"
    url="https://youtu.be/TtncZNCWpQE?si=Ys_SbWAv__wDYzNU" >}}

{{< drg/video
    title="Plaiting: How to Braid Any Number of Cords (video)"
    url="https://youtu.be/qjqXAV9lyns?si=1VmEcBuPoFOW1wFh" >}}

### Braid vs. plait

People sometimes use these words almost interchangeably, but both involve weaving strands in a repeated pattern. The important skill for this requirement is showing that you can organize multiple strands into a strong, neat article such as a bracelet, key fob, zipper pull, dog leash accent, or decorative strap.

### Keys to a good result

Keep strand widths consistent. Anchor one end before you begin. Pull with even tension after each move. Stop occasionally to straighten the pattern instead of hoping it fixes itself later.

{{< drg/image src="images/four-strand-braid-sequence.avif" alt="Four-panel illustrated sequence of a four-strand leather braid showing strand order changes and the emerging round braid pattern" >}}

{{< drg/checklist title="Braiding habits that help" subtitle="Use these while you work" >}}
- Start with strands that are cut evenly.
- Keep the work anchored or clipped in place.
- Repeat the same sequence patiently.
- Tighten each move a little, not all at once.
- Check the pattern often so twists do not build up unnoticed.
{{< /drg/checklist >}}

{{< drg/be-prepared title="Your braid starts twisting" >}}
- **Pause before tightening farther.**
- **Untwist the working strands gently.**
- **Check whether one strand is pulling much tighter than the others.**
- **Resume with smaller, more even pulls.**
{{< /drg/be-prepared >}}

Req 5 lets you choose one larger direction to explore next: tanning, hide work, care and repair, or learning from a real leather business.

{{< drg/next-page
    text="You have seen how woven strands can become a finished leathercraft article."
    teaser="Next, choose one deeper leather topic to explore beyond the basic shop skills."
    url="/merit-badges/leatherwork/guide/req5/" >}}
