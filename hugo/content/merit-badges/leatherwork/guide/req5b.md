---
title: "Req 5b — Tanning a Small Hide"
layout: guide
group_title: "Exploring Leather Beyond the Bench"
req_number: "5b"
prev: "/merit-badges/leatherwork/guide/req5a/"
prev_title: "Req 5a — Commercial Tanning"
next: "/merit-badges/leatherwork/guide/req5c/"
next_title: "Req 5c — Care, Repair, and Comparison"
---

{{< drg/requirement number="5b" >}}
Tan the skin of a small animal. Describe the safety precautions you will take and the tanning method that you used.
{{< /drg/requirement >}}

This option is hands-on and serious. You are working with an actual hide, biological material, sharp tools, and possibly chemicals depending on the method. That means safety and adult supervision are not side notes here — they are central to the requirement.

{{< drg/video
    title="How to Tan Animal Hides (Never Destroy Animals for This Purpose) (video)"
    url="https://youtu.be/RJNyABFzWPw?si=TqYic4EWmIBT7Zzx" >}}

### What your counselor will care about

You should be able to explain where the hide came from, what method you used, what steps you followed, and how you handled sanitation and safety. The exact tanning method may vary, but the important part is understanding the process rather than treating it like a mystery recipe.

### Safety precautions to describe

{{< drg/safety-first >}}
Use gloves, protect the work surface, wash tools thoroughly, and follow any instructions for chemicals exactly as written. Never use a hide from an unsafe or questionable source, and never kill an animal just to complete this requirement.
{{< /drg/safety-first >}}

Sanitation matters because raw hides can carry bacteria. Ventilation matters if any chemical treatment is involved. Tool safety matters during scraping, trimming, or fleshing. If you used a particular tanning solution, be ready to explain how you handled storage, disposal, and skin protection.

{{< drg/be-prepared title="Your hide starts smelling bad or decomposing" >}}
- **Stop and assess the condition honestly.**
- **Tell an adult or your counselor.**
- **Review whether the hide was preserved, chilled, salted, or processed soon enough.**
- **Do not keep handling questionable material casually.**
{{< /drg/be-prepared >}}

Req 5c moves away from raw hides and toward a skill many Scouts will use more often in everyday life: leather care.

{{< drg/next-page
    text="You know what makes small-scale tanning a serious, safety-focused process."
    teaser="Next, learn how to care for finished leather goods and compare leather with synthetics."
    url="/merit-badges/leatherwork/guide/req5c/" >}}
