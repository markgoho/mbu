---
title: "Req 5 — Build Your Four-Week Plan"
layout: "guide"
group_title: "Build Your Plan"
req_number: "5"
req_path: "5"
prev: "/merit-badges/multisport/guide/req4d2/"
prev_title: "Req 4d2 — Aquabike Bike Skills"
next: "/merit-badges/multisport/guide/req6/"
next_title: "Req 6 — Set Up Your Transition Area"
---

{{< drg/requirement number="5" >}}
Do the following:
{{< /drg/requirement >}}

This requirement turns practice into a plan. You will design four weeks of training, track what you do, set a personal goal, and reflect on how you changed.

## Requirement 5a

{{< drg/requirement number="5a" >}}
With guidance from your counselor, establish a four-week training plan that combines your chosen multisport format to develop proper techniques, gain self-confidence, and increase endurance. Each session should last at least 25 minutes and include a proper warmup before the session and stretching afterward.
{{< /drg/requirement >}}

A good training plan is realistic, not heroic. It should match your current level, use the format you chose in Requirement 3d, and give you enough work to improve without overloading your body.

{{< drg/download
    title="Multisport Training Plan Worksheet"
    url="/merit-badges/multisport/guide/training-plan-worksheet/"
    type="printable" >}}

A simple four-week plan usually includes:

- **2–3 focused sessions each week**
- **At least one easier or recovery day between harder efforts**
- **Practice in the disciplines your format requires**
- **A clear skill focus** such as pacing, transitions, swimming comfort, or bike handling

{{< drg/video
    title="Advice for Younger Athletes | Our Tips For Junior And Youth Triathletes (video)"
    url="https://youtu.be/PxVw1eZp7d4" >}}

{{< drg/video
    title="How to Start Swim Training (video)"
    url="https://youtu.be/Zh9bhXe5lug" >}}

{{< drg/video
    title="Is There a Perfect Swimming Technique for Triathlon? (video)"
    url="https://youtu.be/i6-w8RXf45Y" >}}

{{< drg/video
    title="How To Start Running Training (video)"
    url="https://youtu.be/kVnyY17VS9Y" >}}

## Requirement 5b

{{< drg/requirement number="5b" >}}
Use a chart or other tracking method to monitor your training and development during this period.
{{< /drg/requirement >}}

Tracking matters because memory is unreliable. If you only guess how often you trained or how well you felt, it is hard to see progress clearly.

A good chart can include:

- date
- session type
- time or distance
- skill focus
- how you felt physically
- what improved

{{< drg/checklist title="What to record after each session" subtitle="Keep your chart useful, not fancy" >}}
- **What you did**
- **How long it lasted**
- **One thing that felt better**
- **One thing that still needs work**
{{< /drg/checklist >}}

## Requirement 5c

{{< drg/inherited-requirement number="5c" req_path="5.c" topic="Set a personal goal for improvement based on one or more of the following criteria: time, technique, or distance." />}}

### Goal based on time

You might aim to complete a training segment faster while still staying in control.

### Goal based on technique

You might aim to improve something like smoother breathing in the water, cleaner bike starts, or better running posture.

### Goal based on distance

You might aim to cover more distance comfortably within your session.

{{< drg/tip >}}
Choose a goal you can influence directly. “Improve my swim sighting on every open-water practice” is often better than a huge time-drop goal with too many variables.
{{< /drg/tip >}}

## Requirement 5d

{{< drg/inherited-requirement number="5d" req_path="5.d" topic="At the end of four weeks, discuss your progress with your counselor and tell how your development has affected you mentally and physically." />}}

### Physical effects to notice

You may feel stronger, less tired at the same effort, more coordinated, or more comfortable moving from one discipline to another.

### Mental effects to notice

You may feel more confident, more patient, or more willing to tackle challenges that seemed intimidating before.

### How to talk about progress honestly

Progress does not have to mean “everything became easy.” It may mean you learned what still needs work and now have a smarter plan.

{{< drg/be-prepared title="If your plan goes off track" >}}
- **Adjust, do not quit.** Missing a few sessions does not ruin the whole month.
- **Tell your counselor what changed.**
- **Return to consistency first,** then intensity.
- **Protect recovery** if you start feeling worn down or sore in a bad way.
{{< /drg/be-prepared >}}

Your next requirement is about transitions, where organization and calm decision-making can make a huge difference on event day.

{{< drg/next-page
    text="You now know how to build and track a real four-week plan."
    teaser="Next, set up a transition area that helps you move smoothly and safely from one discipline to the next."
    url="/merit-badges/multisport/guide/req6/" >}}
