---
title: "Req 8c — Lead a Training Group"
layout: "guide"
group_title: "Staying Involved"
req_number: "8c"
req_path: "8.c"
prev: "/merit-badges/multisport/guide/req8b/"
prev_title: "Req 8b — Study a Triathlete"
next: "/merit-badges/multisport/guide/req8d/"
next_title: "Req 8d — Enter a Sanctioned Event"
---

{{< drg/requirement number="8c" >}}
Demonstrate leadership by starting a training group and educating your peers on the importance of physical activity, nutrition, and the disciplines of multisport.
{{< /drg/requirement >}}

Leadership here does not mean acting like an expert coach. It means organizing people, keeping the activity safe, and helping others learn something useful.

### A simple way to lead a training group

- choose a clear purpose for the session
- pick one or two topics to teach
- keep the activity beginner-friendly
- explain safety rules before movement starts
- encourage everyone, not just the strongest athletes

{{< drg/video
    title="Training With a Club (video)"
    url="https://www.youtube.com/watch?v=ogogU_qZok8%20" >}}

{{< drg/be-prepared title="What to teach in a first training-group session" >}}
- **Physical activity:** why regular movement helps health and confidence
- **Nutrition:** simple fueling and hydration basics
- **Disciplines of multisport:** what swimming, biking, and running each demand
- **Safety:** the rules that come before speed
{{< /drg/be-prepared >}}

{{< drg/tip >}}
The best youth leaders make a session feel welcoming. If beginners leave wanting to come back, you led well.
{{< /drg/tip >}}

Next, you can look at joining a real sanctioned event and seeing how official race structure works.

{{< drg/next-page
    text="You know how to turn your badge experience into leadership."
    teaser="Next, explore what it takes to register for and participate in a sanctioned multisport event."
    url="/merit-badges/multisport/guide/req8d/" >}}
