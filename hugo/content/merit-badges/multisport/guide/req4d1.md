---
title: "Req 4d1 — Aquabike Swim Skills"
layout: "guide"
group_title: "Aquabike Path"
req_number: "4d1"
req_path: "4.d.1"
prev: "/merit-badges/multisport/guide/req4d/"
prev_title: "Req 4d — Aquabike Roadmap"
next: "/merit-badges/multisport/guide/req4d2/"
next_title: "Req 4d2 — Aquabike Bike Skills"
---

{{< drg/requirement number="4d1" >}}
Swimming
{{< /drg/requirement >}}

The aquabike swim section uses the same swimming foundation as triathlon and aquathlon. You still need the prerequisite badge, Safe Swim Defense knowledge, and an understanding of open-water conditions.

## Requirement 4d1a

{{< drg/requirement number="4d1a" >}}
Before doing requirements 5 through 8, earn the Swimming merit badge.
{{< /drg/requirement >}}

The Swimming merit badge proves you already have the water-safety baseline needed before you combine swimming with the stress of a multisport event.

{{< drg/external-link
    title="Swimming Merit Badge (website)"
    url="https://www.scouting.org/merit-badges/swimming/%20"
    description="Use the official Swimming merit badge page to review the prerequisite skills that support safe participation in the aquabike branch." >}}

## Requirement 4d1b

{{< drg/requirement number="4d1b" >}}
Explain the components of the Scouting America Safe Swim Defense guidelines and how you will ensure they are in place when you swim.
{{< /drg/requirement >}}

You should be able to explain supervision, safe areas, ability groups, buddy systems, and emergency readiness in a way that connects to your actual swim setting.

{{< drg/safety-first >}}
If the swim setting is poorly supervised, poorly marked, or beyond your ability, the correct decision is not to continue just because the rest of your event plan looks exciting.
{{< /drg/safety-first >}}

## Requirement 4d1c

{{< drg/requirement number="4d1c" >}}
Explain to your counselor the difference between a pool swim and an open water swim, including at what water temperature it is appropriate to wear a wet suit.
{{< /drg/requirement >}}

Open water demands more navigation, more comfort with uncertainty, and more attention to temperature than a pool swim. Wetsuit use depends on event rules and conditions, not just preference.

{{< drg/video
    title="4 Things To Know Before Your First Open Water Swim (video)"
    url="https://youtu.be/0VN9xikjs2E" >}}

{{< drg/video
    title="The Dangers of Cold Water Immersion (video)"
    url="https://youtu.be/fLdw5NBOf6g" >}}

{{< drg/video
    title="How Do Wetsuits Keep You Warm? (video)"
    url="https://youtu.be/6B05sU20-Jw" >}}

Next, you will shift from the swim to the bike, where control and equipment checks become the focus.

{{< drg/next-page
    text="You have the aquabike swim basics in place."
    teaser="Now move to the bike section and build the cycling habits that complete this branch."
    url="/merit-badges/multisport/guide/req4d2/" >}}
