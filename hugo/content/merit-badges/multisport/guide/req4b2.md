---
title: "Req 4b2 — Duathlon Run Skills"
layout: "guide"
group_title: "Duathlon Path"
req_number: "4b2"
req_path: "4.b.2"
prev: "/merit-badges/multisport/guide/req4b1/"
prev_title: "Req 4b1 — Duathlon Bike Skills"
next: "/merit-badges/multisport/guide/req4c/"
next_title: "Req 4c — Aquathlon Roadmap"
---

{{< drg/requirement number="4b2" >}}
Running
{{< /drg/requirement >}}

Duathlon begins and ends with running, so efficient form and smart pacing matter even more. The first run should leave something in the tank for the bike and the final run.

## Requirement 4b2a

{{< drg/requirement number="4b2a" >}}
Demonstrate a proper run warmup and cool-down. Explain to your counselor the importance of maintaining healthy habits, including hydration, nutrition, injury prevention, and rest.
{{< /drg/requirement >}}

A good warmup prepares you to run well from the first minutes instead of spending the first part of the session feeling stiff and awkward. A cooldown helps you recover more smoothly.

{{< drg/video
    title="Warm-Up for Running (video)"
    url="https://youtu.be/PkF3jjFZc0s" >}}

{{< drg/video
    title="Hydration for Running (video)"
    url="https://youtu.be/MpCO8Y7iPYk" >}}

## Requirement 4b2b

{{< drg/inherited-requirement number="4b2b" req_path="4.b.2.b" topic="Learn and state the basic rules of the road for runners." />}}

### Position on the road

Use sidewalks or separated paths when available. Without them, runners should generally face traffic so they can see approaching vehicles.

### Visibility

Choose bright or reflective gear when light is low, and assume drivers may not notice you right away.

### Awareness

Watch intersections, parked cars, driveways, and cyclists. Safe runners pay attention even when tired.

{{< drg/video
    title="Safety Tips for Running (video)"
    url="https://youtu.be/I6aDhB_uC8A" >}}

## Requirement 4b2c

{{< drg/requirement number="4b2c" >}}
Demonstrate important running drills, including high knees, butt kicks, lunges, inchworms, and soldier kicks.
{{< /drg/requirement >}}

These drills help you move better, not just warm up longer. They support posture, stride rhythm, balance, and flexibility.

{{< drg/video
    title="High Knees and Butt Kicks (video)"
    url="https://youtu.be/tY2bLt7roOQ" >}}

{{< drg/video
    title="Lunges (video)"
    url="https://youtu.be/amElt61OHgM" >}}

{{< drg/video
    title="Inchworms (video)"
    url="https://youtu.be/pv_8CdDPAAk" >}}

{{< drg/video
    title="Soldier Kicks (video)"
    url="https://youtu.be/9cacAKZESf0" >}}

{{< drg/tip >}}
In duathlon, starting too hard on the first run can make the bike leg feel much worse than it should. Practice effort control, not just effort output.
{{< /drg/tip >}}

The next branch shows aquathlon, where the swim-to-run transition creates a different kind of challenge.

{{< drg/next-page
    text="You have the duathlon run skills in place."
    teaser="Next, see how aquathlon combines water confidence with efficient running."
    url="/merit-badges/multisport/guide/req4c/" >}}
