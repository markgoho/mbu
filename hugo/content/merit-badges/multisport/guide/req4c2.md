---
title: "Aquathlon Run Skills"
layout: "guide"
group_title: "4. Aquathlon Path"
req_number: "4c2"
req_path: "4.c.2"
prev: "/merit-badges/multisport/guide/req4c1/"
prev_title: "Aquathlon Swim Skills"
next: "/merit-badges/multisport/guide/req4d/"
next_title: "Aquabike Roadmap"
---

{{< drg/requirement number="4c2" >}}
Running
{{< /drg/requirement >}}

In aquathlon, the run begins after the swim, so your legs may feel strange at first and your breathing may still be settling. Good run habits help you regain rhythm quickly.

## Requirement 4c2a

{{< drg/requirement number="4c2a" >}}
Demonstrate a proper run warmup and cool-down. Explain to your counselor the importance of maintaining healthy habits, including hydration, nutrition, injury prevention, and rest.
{{< /drg/requirement >}}

Warmups prepare you to move well. Cooldowns help you recover. Hydration, nutrition, injury prevention, and rest make those efforts sustainable across weeks of training.

{{< drg/video
    title="Warm-Up for Running (video)"
    url="https://youtu.be/PkF3jjFZc0s" >}}

{{< drg/video
    title="Hydration for Running (video)"
    url="https://youtu.be/MpCO8Y7iPYk" >}}

## Requirement 4c2b

{{< drg/inherited-requirement number="4c2b" req_path="4.c.2.b" topic="Learn and state the basic rules of the road for runners." />}}

### Safe positioning

Use sidewalks or paths when possible. Without them, face traffic so you can see approaching vehicles.

### Safe visibility

Wear bright or reflective gear and expect low-light conditions to make you harder to notice.

### Safe awareness

Stay alert at driveways, intersections, and course crossings. Tired runners still need good judgment.

{{< drg/video
    title="Safety Tips for Running (video)"
    url="https://youtu.be/I6aDhB_uC8A" >}}

## Requirement 4c2c

{{< drg/requirement number="4c2c" >}}
Demonstrate important running drills, including high knees, butt kicks, lunges, inchworms, and soldier kicks.
{{< /drg/requirement >}}

These drills build coordination, mobility, and better movement patterns.

{{< drg/video
    title="High Knees and Butt Kicks (video)"
    url="https://youtu.be/tY2bLt7roOQ" >}}

{{< drg/video
    title="Lunges (video)"
    url="https://youtu.be/amElt61OHgM" >}}

{{< drg/video
    title="Inchworms (video)"
    url="https://youtu.be/pv_8CdDPAAk" >}}

{{< drg/video
    title="Soldier Kicks (video)"
    url="https://youtu.be/9cacAKZESf0" >}}

{{< drg/tip >}}
If your first steps out of the water feel awkward, shorten your stride for a moment and let rhythm return naturally instead of forcing speed right away.
{{< /drg/tip >}}

The next branch is aquabike, where the swim leads into the bike instead of the run.

{{< drg/next-page
    text="You have completed the aquathlon run section."
    teaser="Next, explore aquabike and see how the swim-to-bike transition changes the focus."
    url="/merit-badges/multisport/guide/req4d/" >}}
