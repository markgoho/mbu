---
title: "Road Biking Option"
layout: guide
group_title: "6. Your Rides (Choose One Option)"
req_number: "6a"
prev: "/merit-badges/cycling/guide/req5/"
prev_title: "Basic Bike Handling"
next: "/merit-badges/cycling/guide/req6b/"
next_title: "Trail Biking Option"
---

{{< drg/requirement number="6" option="Road Biking Option" >}}
Using the Scouting America buddy system, complete all of the requirements for ONE of the following options: Road Biking OR Trail or Mixed Surface Biking.
{{< /drg/requirement >}}

If you prefer trail or mixed-surface riding, see [Req 6b — Trail Biking Option](/merit-badges/cycling/guide/req6b/). You only need to complete **one** of the two options.

{{< drg/safety-first >}}
All rides for Requirement 6 must be done using the Scouting America buddy system. Never ride alone. Plan your routes in advance and share them with a responsible adult.
{{< /drg/safety-first >}}

---

### Requirement 6a.1 — Road Safety Test

{{< drg/requirement number="6a.1" >}}
Take a road safety test with your counselor and demonstrate the following:
{{< /drg/requirement >}}

Your counselor will ride with you (or observe you) on public streets to evaluate your road-riding skills. This is a practical test — you will need to demonstrate each skill listed below in real traffic conditions.

#### 6a.1.a — Left Turns

{{< drg/requirement number="6a.1.a" >}}
On an urban street with light traffic, properly execute a left turn from the center of the street; also demonstrate an alternate left-turn technique used during periods of heavy traffic.
{{< /drg/requirement >}}

**Standard vehicular left turn:**
1. Scan behind you for traffic.
2. Signal left with your left arm extended.
3. When safe, merge to the center of the lane (or into the left-turn lane if one exists).
4. Yield to oncoming traffic.
5. Complete the turn into the proper lane.

**Box turn (pedestrian-style left turn) for heavy traffic:**
1. Ride straight through the intersection in the right lane.
2. Stop on the far-right corner, staying out of the crosswalk.
3. Reposition your bike to face the new direction of travel.
4. Wait for the green signal, then proceed across the intersection.

{{< drg/tip >}}
The box turn is your safest option on busy multi-lane roads. You never have to merge into fast traffic or cross multiple lanes. Practice both techniques so you can choose the right one for each situation.
{{< /drg/tip >}}

#### 6a.1.b — Right Turns

{{< drg/requirement number="6a.1.b" >}}
Properly execute a right turn.
{{< /drg/requirement >}}

1. Scan behind you.
2. Signal right with your right arm.
3. Check for pedestrians in the crosswalk.
4. Make the turn from the right side of the lane, staying in control.

#### 6a.1.c — Right-Turn-Only Lanes

{{< drg/requirement number="6a.1.c" >}}
Demonstrate appropriate actions at a right-turn-only lane when you are continuing straight.
{{< /drg/requirement >}}

When a right-turn-only lane appears and you need to go straight:

1. Before the lane splits, merge left out of the right-turn-only lane.
2. Signal your intention early.
3. Position yourself in the through-traffic lane.
4. Proceed straight through the intersection.

{{< drg/safety-first >}}
This maneuver requires you to merge into traffic. Scan, signal, and make eye contact with drivers behind you. If traffic is heavy and you cannot merge safely, use the sidewalk or crosswalk as a pedestrian — dismount and walk your bike across.
{{< /drg/safety-first >}}

#### 6a.1.d — Curbside and Road-Edge Riding

{{< drg/requirement number="6a.1.d" >}}
Show proper curbside and road-edge riding. Show how to ride safely along a row of parked cars.
{{< /drg/requirement >}}

- Ride at least 4 feet from parked cars to stay outside the "door zone."
- Maintain a straight, predictable line — don't weave in and out of gaps between parked cars.
- Scan for signs that a car door may open: brake lights, people sitting inside, or movement in side mirrors.

#### 6a.1.e — Railroad Tracks

{{< drg/requirement number="6a.1.e" >}}
Cross railroad tracks properly.
{{< /drg/requirement >}}

- Approach railroad tracks at a right angle (perpendicular). If the tracks cross the road diagonally, adjust your line so you cross them straight-on.
- Rise slightly off the saddle to absorb the bump.
- Do not brake on the tracks — metal rails are slippery, especially when wet.

{{< drg/did-you-know >}}
Railroad tracks are one of the most common causes of cycling crashes. A tire that enters a rail groove at a shallow angle can get trapped, causing an instant loss of control. The perpendicular approach eliminates this risk.
{{< /drg/did-you-know >}}

{{< drg/image src="images/road-safety-skills.avif" alt="A Scout cycling on a suburban street demonstrating proper lane position, passing a row of parked cars with safe clearance" >}}

---

### Requirement 6a.2 — Building Your Miles

{{< drg/requirement number="6a.2" >}}
Avoiding main highways, take two rides of 10 miles each, two rides of 15 miles each, and two rides of 25 miles each. You must make a report of the rides taken. List dates for the routes traveled, and interesting things seen on the ride.
{{< /drg/requirement >}}

This is where the real adventure begins. Six rides totaling 100 miles will build your endurance, confidence, and love of cycling. The rides get progressively longer, giving your body time to adapt.

{{< drg/download
    title="Ride Log"
    url="/merit-badges/cycling/guide/ride-log/"
    type="printable" >}}

#### Planning Your Rides

{{< drg/checklist title="Ride Planning" subtitle="Set yourself up for success" >}}
- Map your route in advance using a cycling app or a paper map. Stay on secondary roads, bike lanes, and multi-use paths — avoid main highways.
- Share your route and estimated return time with an adult.
- Ride with your buddy — it's required, and it makes the ride safer and more fun.
- Check the weather and plan accordingly.
- Carry water, snacks, a repair kit, your phone, and your first-aid kit.
{{< /drg/checklist >}}

#### Pacing Yourself

| Ride Distance | Estimated Time (10–12 mph) | Tips |
|---------------|---------------------------|------|
| 10 miles | 50–60 minutes | A great training ride. Focus on smooth pedaling and consistent speed. |
| 15 miles | 75–90 minutes | Bring a snack and extra water. Practice fueling while riding. |
| 25 miles | 2–2.5 hours | Plan a rest stop at the halfway point. Eat and drink before you feel hungry or thirsty. |

{{< drg/tip >}}
Start your longer rides early in the morning when temperatures are cooler, traffic is lighter, and you have plenty of daylight. Eat a good breakfast 1–2 hours before you ride.
{{< /drg/tip >}}

#### Ride Reports

Your report for each ride should include:

- Date of the ride
- Route taken (description or map)
- Total distance and approximate time
- Weather conditions
- Interesting things you saw or experienced
- Any challenges or lessons learned

---

### Requirement 6a.3 — The Capstone Ride

{{< drg/requirement number="6a.3" >}}
After completing requirement (2) for the road biking option, do ONE of the following:
{{< /drg/requirement >}}

This is the final challenge of the road biking option. Choose **one** of these two sub-options:

#### Option 6a.3.a — Plan and Ride a 50-Mile Route

{{< drg/requirement number="6a.3.a" >}}
Lay out on a road map a 50-mile trip. Stay away from main highways. Using your map, make this ride in eight hours or less.
{{< /drg/requirement >}}

A 50-mile ride (often called a "half-century") is a major accomplishment. Planning is everything.

{{< drg/checklist title="50-Mile Ride Planning" subtitle="Prepare for the big ride" >}}
- Plan your route on a road map. Mark rest stops every 15–20 miles where you can refuel and use a restroom.
- Carry enough food and water — or plan stops where you can buy supplies.
- Start early. At 12 mph average speed, a 50-mile ride takes about 4 hours of riding time plus stops.
- Break the ride into mental segments: "Just 10 more miles to the next rest stop."
- Bring your map and follow your planned route. Navigation is part of the skill.
{{< /drg/checklist >}}

#### Option 6a.3.b — Organized Bike Tour

{{< drg/requirement number="6a.3.b" >}}
Participate in an organized bike tour of at least 50 miles. Make this ride in eight hours or less. Afterward, use the tour's cue sheet to make a map of the ride.
{{< /drg/requirement >}}

Organized rides provide rest stops, route markings, and safety support, making them an excellent way to complete your first 50-mile ride. After the ride, you will use the event's cue sheet (turn-by-turn directions) to create a map showing the route you followed.

{{< drg/did-you-know >}}
A "cue sheet" is a turn-by-turn direction list used in organized cycling events. It lists every turn, the distance to each turn, and landmarks to watch for. Learning to follow a cue sheet is a classic cycling skill.
{{< /drg/did-you-know >}}

{{< drg/image src="images/road-cycling-scenic.avif" alt="Two Scouts riding road bikes along a scenic rural road with rolling hills and a clear sky" >}}

### Explore More Resources

{{< drg/video title="Advanced Techniques for Urban Riding | Scouting America" url="https://vimeo.com/1125028086" >}}

{{< drg/external-link title="MapMyRide" url="https://www.mapmyride.com/" description="Plan and track your cycling routes. Great for mapping your required rides." >}}

{{< drg/external-link title="Bikeability Checklist (NHTSA)" url="https://www.nhtsa.gov/sites/nhtsa.gov/files/bikabilitychecklist1.pdf" description="Evaluate the bikeability of your community — useful for planning safe routes." >}}

{{< drg/next-page
    text="That covers the road biking option."
    teaser="Check out the trail biking option to see the other path you can choose."
    url="/merit-badges/cycling/guide/req6b/" >}}
