---
title: "Req 3 — Choose Your Field Study"
layout: "guide"
group_title: "Field Study Options"
req_number: "3"
req_path: "3"
prev: "/merit-badges/mammal-study/guide/req2/"
prev_title: "Req 2 — Classifying Mammals"
next: "/merit-badges/mammal-study/guide/req3a/"
next_title: "Req 3a — Two Habitats, Six Hours"
---

{{< drg/requirement number="3" >}}
Do ONE of the following:
{{< /drg/requirement >}}

This requirement gives you three very different ways to study mammals. You will pick exactly one option. One path emphasizes comparing habitats, one rewards repeated visits to the same place, and one is a research-based life-history project.

## Your Options

- **[Req 3a — Two Habitats, Six Hours](/merit-badges/mammal-study/guide/req3a/)**: Visit two different habitats or elevations, record mammals by sight or sign, compare what you found, and explain why species do not all live in the same place. This option builds observation and comparison skills.
- **[Req 3b — Five-Day Observation Log](/merit-badges/mammal-study/guide/req3b/)**: Return to the same 4-acre area on five different days for a longer total of 15 hours. This option teaches patience, repeat observation, and how mammal activity changes over time.
- **[Req 3c — Nongame Mammal Life History](/merit-badges/mammal-study/guide/req3c/)**: Research one nongame mammal in your area and write about how it lives, what it eats, how it reproduces, and how it relates to people and its ecosystem. This option is best if you enjoy reading, writing, and connecting facts into a bigger story.

## How to Choose

{{< drg/checklist title="Choosing the Right Option" subtitle="Match the option to your situation" >}}
- **Time outside:** Option 3a takes two longer outings; Option 3b takes five separate visits; Option 3c can be done mostly through research.
- **Access to habitat:** Option 3a works well if you can reach two clearly different places such as woods and wetland, or valley and ridge. Option 3b works best if you have one safe area you can revisit easily.
- **Best for note takers:** Option 3b is excellent if you like keeping a log and spotting patterns over time.
- **Best for strong readers and writers:** Option 3c fits Scouts who enjoy research, comparing sources, and writing a structured report.
- **What you gain:** Option 3a strengthens habitat comparison, Option 3b builds long-term observation habits, and Option 3c develops research and ecological storytelling skills.
{{< /drg/checklist >}}

### A closer look at the trade-offs

Option 3a is often the most balanced choice. You get real field experience without needing five separate trips. It is especially strong if your area has easy access to different habitat types.

Option 3b is great for Scouts who want to understand one place deeply. Returning to the same area teaches you that wildlife changes with weather, time of day, season, and human disturbance.

Option 3c is a strong option when travel is limited or when you already have a favorite local species you want to understand better. It also prepares you well for [Req 4c](/merit-badges/mammal-study/guide/req4c/), which uses a similar life-history approach for a game mammal.

{{< drg/tip >}}
If you are undecided, ask yourself one simple question: do you want your evidence to come mostly from outdoor observation or from reading and writing? Choose the option that matches the kind of work you are most likely to finish well.
{{< /drg/tip >}}

Start with the first option page even if you may choose a different one. It will show you the kind of careful observation this badge values.

{{< drg/next-page
    text="You have three good paths in front of you"
    teaser="First, see how a two-habitat field study works and what strong notes look like."
    url="/merit-badges/mammal-study/guide/req3a/" >}}
