---
title: "Req 4d — Tracking Pit Investigation"
layout: "guide"
group_title: "Mammal Investigation Options"
req_number: "4d"
req_path: "4.d"
prev: "/merit-badges/mammal-study/guide/req4c/"
prev_title: "Req 4c — Game Mammal Life History"
next: "/merit-badges/mammal-study/guide/req4e/"
next_title: "Req 4e — Natural History Museum Visit"
---

{{< drg/requirement number="4d" >}}
Make and bait a tracking pit. Report what mammals and other animals came to the bait.
{{< /drg/requirement >}}

A tracking pit is a simple field experiment. Instead of trying to chase wildlife, you prepare a surface that can record visits while you are away. When it works, you get a snapshot of what passed through and sometimes even how it moved.

## What a Tracking Pit Does

A tracking pit creates a soft, readable surface such as smooth sand, sifted soil, or fine dust. Bait or scent attracts interest, and the surface records footprints. You are not trapping the animal. You are collecting evidence.

## Picking a Good Site

The best sites are places animals already travel:

- edge of brush and open ground
- near a log, fence line, or game trail
- close to water or cover
- quiet backyard corners with little disturbance

Choose a legal, safe location where you can check the pit without damaging habitat.

## What to Report

Your report should include:

- where you placed the pit
- what bait you used
- weather or surface conditions
- what tracks or other evidence appeared
- which animals likely visited
- how confident you are in each identification

## Official Resources

These official videos show several ways to set up a track station and interpret what you see.

{{< drg/video
    title="Build Animal Track Station (video)"
    url="https://www.youtube.com/watch?v=rjg7FUw_5fM" >}}

{{< drg/video
    title="Creating a Backyard Animal Track Trap With Household Items (video)"
    url="https://www.youtube.com/watch?v=j_SPxHCvSYA" >}}

{{< drg/video
    title="Animal Tracking Basics - Track Traps, Measurements, Following Sign (video)"
    url="https://www.youtube.com/watch?v=i2SFVKWhIzY" >}}

{{< drg/checklist title="Tracking Pit Notes" subtitle="Record these so your report is useful" >}}
- Date pit was made
- Exact location and habitat
- Surface material used
- Type of bait or scent used
- Dates and times checked
- Track sketches or photos
- Best identification for each visitor
{{< /drg/checklist >}}

{{< drg/be-prepared title="When the Tracks Are Messy" >}}
Tracks do not have to be perfect to be useful.

- **Look for pattern**: Was the animal walking, hopping, or circling the bait?
- **Measure anyway**: Length and width can still help.
- **Check for extra clues**: Tail drags, claw marks, scat, or disturbed vegetation may help with identification.
- **Admit uncertainty**: It is fine to say “small canid” or “possible raccoon” if the evidence is incomplete.
{{< /drg/be-prepared >}}

{{< drg/safety-first >}}
Do not bait sites in ways that attract dangerous wildlife into unsafe areas or encourage animals to depend on human food. Follow local rules, avoid harmful substances, and place your pit where people, pets, and wildlife can remain safe.
{{< /drg/safety-first >}}

The next option shifts from field evidence to collections work and asks what museums do with the specimens they hold.

{{< drg/next-page
    text="You know how to design a simple track-station experiment"
    teaser="Next, look inside a natural history museum and see how specimens are prepared and cataloged."
    url="/merit-badges/mammal-study/guide/req4e/" >}}
