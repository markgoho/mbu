---
title: "Learn from Pottery in Person"
layout: "guide"
group_title: "7. Choose Your Pottery Experience"
req_number: "7b"
req_path: "7.b"
prev: "/merit-badges/pottery/guide/req7a/"
prev_title: "Visit a Kiln Yard"
next: "/merit-badges/pottery/guide/req7c/"
next_title: "Pottery in History and Culture"
---

{{< drg/requirement number="7b" >}}
Visit a museum, art exhibit, art gallery, artists' co-op, or artist's studio that features pottery. After your visit, share with your counselor what you have learned.
{{< /drg/requirement >}}

Pottery looks different when it is right in front of you. Photographs flatten scale, texture, and surface in a way real life does not. This option is about learning to observe carefully, not just strolling past display cases.

## What to notice during your visit

### Form

How is the piece shaped? Is it tall and narrow, wide and stable, or balanced in a surprising way? Ask what the form suggests before you even read the label.

### Surface

Look closely at glaze, carving, paint, texture, burnishing, or firing marks. Does the surface feel smooth, rough, glossy, smoky, cracked, layered, or intentional in some other way?

### Function or purpose

Can you tell whether the object was made for use, ceremony, storage, display, or storytelling? Some pieces are clearly functional. Others are more like sculpture.

### Context

What does the label, gallery note, or artist statement tell you about where the work came from? Pottery often makes more sense when you know the culture, time period, or studio tradition behind it.

{{< drg/be-prepared title="If a gallery visit feels intimidating" >}}
Treat it like a detective mission instead of an art test.

- **Start with one piece** that catches your eye.
- **Describe what you actually see** before reading the label.
- **Read the context** and compare it with your first impression.
- **Write down one question** the piece raises for you.
  {{< /drg/be-prepared >}}

## Good notes to bring back

Your counselor discussion will be stronger if you record more than "I liked it." Try to bring back notes like these:

- Name of the place you visited
- Date of the visit
- One or two specific pieces you studied
- What stood out about form, decoration, or firing
- What you learned from labels, guides, or artists
- One idea you might borrow in your own pottery

{{< drg/external-link
    title="Ceramics in the US (website)"
    url="https://ceramic.school/location/united-states/"
    description="A directory of ceramics-related places and events that may help you find a museum, studio, or exhibit near you." >}}

{{< drg/tip >}}
If the place allows it, sketch one piece instead of only taking photos. Sketching slows you down and forces you to notice proportions, handles, rims, and surface details.
{{< /drg/tip >}}


The last option in Req 7 works well if you want to focus on history and cultural meaning instead of an in-person visit.

{{< drg/next-page
    text="You know how to turn a pottery visit into careful observation and useful notes."
    teaser="Next, explore the research option on pottery's historical and cultural importance."
    url="/merit-badges/pottery/guide/req7c/" >}}
