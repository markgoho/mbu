---
title: "Req 6b — International Space Station"
layout: "guide"
group_title: "Spacecraft Choices"
req_number: "6b"
req_path: "6.b"
prev: "/merit-badges/space-exploration/guide/req6a/"
prev_title: "Req 6a — Crewed Orbital Vehicles"
next: "/merit-badges/space-exploration/guide/req7/"
next_title: "Req 7 — Design a Space Base"
---

{{< drg/requirement number="6b" >}}
International Space Station
{{< /drg/requirement >}}

The International Space Station is both a spacecraft and a place to live. Its purpose is to support long-term human life in orbit while giving scientists a laboratory where they can study microgravity, Earth, technology, and the human body.

## Purpose

The ISS helps crews perform research that is hard or impossible to do on Earth. It also teaches engineers how to support people in space for months at a time, which matters for future missions to the Moon and Mars.

## Operation

The ISS circles Earth about once every 90 minutes. Crews arrive on spacecraft, dock, live aboard, perform experiments, exercise to stay healthy, maintain equipment, and eventually rotate home while a new crew takes over.

Because it is a station instead of a short-mission vehicle, its operation depends on constant teamwork: ground controllers, cargo deliveries, visiting spacecraft, maintenance schedules, and international coordination.

## Components to describe

{{< drg/checklist title="Major ISS systems" subtitle="These components help it work as a long-term orbiting outpost" >}}

- **Modules**: Pressurized rooms where crews live, work, and do science.
- **Solar arrays**: Large panels that collect sunlight for power.
- **Trusses and radiators**: Structures that support equipment and release extra heat.
- **Docking ports**: Places where visiting spacecraft connect.
- **Life-support systems**: Air, water recycling, and temperature control.
- **Robotic systems**: Arms and tools used for moving equipment and helping with station work.
  {{< /drg/checklist >}}

{{< drg/external-link
    title="What is the International Space Station? (website)"
    url="https://www.nasa.gov/learning-resources/for-kids-and-students/what-is-the-international-space-station-grades-5-8/"
    description="A plain-language overview of what the ISS is, what it does, and why it matters." >}}

{{< drg/external-link
    title="How to Spot the International Space Station (ISS) (website)"
    url="https://www.nasa.gov/spot-the-station/"
    description="Use NASA's tracker to spot the ISS overhead and connect this requirement to something you can actually see from Earth." >}}

{{< drg/tip >}}
If you choose the ISS, try spotting it in the sky before talking with your counselor. Seeing it pass overhead makes the station feel real instead of abstract.
{{< /drg/tip >}}

{{< drg/be-prepared title="A strong one-minute ISS explanation" >}}

- **Purpose**: "The ISS is a research lab and living space in orbit." 
- **Operation**: "Crews rotate in and out while the station circles Earth and supports daily science and maintenance." 
- **Components**: "Its modules, solar arrays, docking ports, robotic systems, and life-support equipment all work together to keep people alive and productive." 
  {{< /drg/be-prepared >}}

{{< drg/next-page
    text="You have looked at how people travel through orbit and how they live there for longer missions."
    teaser="Next, use those ideas to design your own inhabited base somewhere else in the solar system."
    url="/merit-badges/space-exploration/guide/req7/" >}}
