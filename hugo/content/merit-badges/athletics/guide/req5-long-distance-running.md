---
title: "Req 5b — Long-Distance Running"
layout: guide
group_title: "Performance Options"
req_number: "5b"
prev: "/merit-badges/athletics/guide/req5-sprinting/"
prev_title: "Req 5a — Sprinting"
next: "/merit-badges/athletics/guide/req5-jumping/"
next_title: "Req 5c — Long Jump or High Jump"
---

{{< drg/requirement number="5" option="Option B—Long-Distance Running" >}}
Complete the activities in FOUR of the following options and show improvement over a three-month period:
{{< /drg/requirement >}}

**Activities:** 3K run (3,000 meters / 1.86 miles) and 5K run (5,000 meters / 3.1 miles).

Long-distance running is the ultimate test of endurance, pacing, and mental toughness. Unlike sprinting, where you give everything in seconds, distance running asks you to manage your effort over a much longer period. The reward? There is nothing quite like the feeling of crossing a finish line after pushing through miles of effort.

### Running Form for Distance

Good running form conserves energy and prevents injury over the long haul:

- **Posture:** Stand tall with a slight forward lean from the ankles (not the waist). Imagine a string pulling you up from the top of your head.
- **Arms:** Bend at roughly 90 degrees. Swing forward and back, not across your body. Relaxed hands — pretend you are holding potato chips without crushing them.
- **Foot strike:** Land with your foot beneath your hips, not out in front of you. Overstriding acts as a brake and stresses your knees and shins.
- **Cadence:** Aim for about 170–180 steps per minute. A higher cadence with shorter strides is generally more efficient and easier on your joints.
- **Breathing:** Breathe rhythmically — many distance runners use a 3:2 pattern (inhale for 3 steps, exhale for 2). Breathe through both your nose and mouth.

### Pacing: The Key to Distance Running

Pacing is the single most important skill in distance running. Going out too fast is the most common mistake new runners make — and it almost always leads to "hitting the wall" later in the race.

{{< drg/be-prepared title="You Went Out Too Fast" >}}
You are one kilometer into your 5K and already gasping for air. Your legs feel heavy and you still have two miles to go.

- **Slow down immediately.** It is better to run slightly slower now than to walk later.
- **Focus on your breathing.** Get it under control before trying to increase your pace.
- **Run the next kilometer at a comfortable, conversational pace** — meaning you could talk in short sentences if you needed to.
- **Lesson for next time:** Start your first kilometer 10–15 seconds slower than your goal pace. You can always speed up in the second half if you feel good.
{{< /drg/be-prepared >}}

### Training for Distance

A solid distance running program includes a mix of easy runs, tempo runs, and one long run per week:

{{< drg/checklist title="Weekly Distance Training Plan" subtitle="A balanced approach for new distance runners" >}}
- Easy runs (2–3 per week): Run at a conversational pace for 20–30 minutes. These build your aerobic base.
- Tempo run (1 per week): Run at a "comfortably hard" pace for 15–20 minutes. This pace is faster than easy but slower than racing — you should be able to speak in short phrases.
- Long run (1 per week): Run farther than your other runs but at an easy pace. Start at 2 miles and add half a mile each week.
- Rest days (1–2 per week): Critical for recovery. Walk, stretch, or do light cross-training instead.
{{< /drg/checklist >}}

{{< drg/tip >}}
The "talk test" is the simplest way to check your pace. If you can carry on a conversation during an easy run, you are at the right speed. If you are gasping for breath, slow down. Most of your training should be at this comfortable, easy effort.
{{< /drg/tip >}}

{{< drg/image src="images/distance-runner-trail.avif" alt="A Scout running on a tree-lined path during a distance run, with a relaxed stride and calm expression" >}}

### 3K vs. 5K: Strategy Differences

**3K (1.86 miles)** is shorter but more intense. You can sustain a faster pace because the distance is more manageable. The challenge is running hard enough without going anaerobic (out of breath) too early.

**5K (3.1 miles)** requires more patience. Your pace will be noticeably slower than your 3K pace. The mental game — staying focused and positive during the middle miles — is often the hardest part.

{{< drg/did-you-know >}}
The 5K is the most popular road race distance in the world. Millions of people — from beginners to elite runners — run 5K races every year. Many communities host weekly "parkrun" events that are free, timed, and open to everyone.
{{< /drg/did-you-know >}}

{{< drg/safety-first >}}
If you experience sharp pain in your shins, knees, or feet during running, stop and rest. Shin splints and stress fractures are overuse injuries that get worse if ignored. Increase your mileage gradually — follow the 10% rule from [Requirement 3](/merit-badges/athletics/guide/req3/).
{{< /drg/safety-first >}}

{{< drg/external-link
    title="Road Runners Club of America"
    url="https://www.rrca.org/"
    description="Find local running clubs, races, and training resources for runners of all levels." >}}

{{< drg/next-page
    text="You have explored the world of distance running."
    teaser="Next, discover the explosive power of jumping — long jump and high jump."
    url="/merit-badges/athletics/guide/req5-jumping/" >}}
