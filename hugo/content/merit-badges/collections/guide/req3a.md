---
title: "Req 3a — Handling, Cleaning & Storage"
layout: guide
group_title: "Preserving & Displaying"
req_number: "3a"
prev: "/merit-badges/collections/guide/req2/"
prev_title: "Req 2 — Growth & Development"
next: "/merit-badges/collections/guide/req3b/"
next_title: "Req 3b — Displaying Your Collection"
---

{{< drg/requirement number="3a" >}}
Explain the precautions you need to take to preserve your collection, including:
{{< /drg/requirement >}}

This requirement covers three essential preservation skills:

- **Handling** — how to touch and move items without causing damage
- **Cleaning** — when and how to clean (and when *not* to)
- **Storage** — protecting your collection from environmental threats

A single careless moment — a fingerprint on a rare coin, a stamp peeled with too much force, a fossil dropped on a hard floor — can permanently reduce an item's condition and value. Preservation is the foundation of good collecting.

## Handling

### The Golden Rule: Less Contact Is Better

Every time you touch a collectible, you risk transferring oils, moisture, and dirt from your skin. Over time, these tiny deposits cause tarnishing, staining, and corrosion.

{{< drg/safety-first >}}
Always wash and dry your hands thoroughly before handling collectibles. For coins, stamps, and photographs, wear clean cotton or nitrile gloves. Hold coins by their edges — never touch the face. Handle stamps with stamp tongs (not regular tweezers, which can crush or crease).
{{< /drg/safety-first >}}

### Surface Protection

Work over a soft, clean surface. A felt pad, a clean towel, or a padded tray prevents damage if you accidentally drop an item. This is especially important for coins, minerals, and figurines.

{{< drg/tip >}}
Keep a pair of cotton gloves and a soft pad in your collection storage area so they are always within reach. If handling items is part of your routine, making it easy to do the right thing means you will actually do it.
{{< /drg/tip >}}

## Cleaning

### The Most Important Rule

**When in doubt, do not clean.** Improper cleaning destroys more collectibles than neglect ever has. Many collectors and grading services actually prefer items in their original, uncleaned state — even if they look a little worn.

| Collection Type | Safe Cleaning Method | Never Do This |
|---|---|---|
| Coins | Gentle rinse in distilled water, pat dry | Polish, scrub, use chemical cleaners |
| Stamps | Light dusting with soft brush | Soak in water unless you know the ink is waterproof |
| Rocks & minerals | Soft brush, distilled water | Use acids without expert guidance |
| Trading cards | Soft microfiber cloth (dry) | Use water or cleaning sprays |
| Fossils | Soft brush, dental pick for matrix | Scrub with hard brushes |

{{< drg/did-you-know >}}
Cleaning a rare coin can reduce its value by 50% or more. Professional grading services like PCGS and NGC will mark a coin as "cleaned" on its certification — and collectors pay significantly less for cleaned coins than for originals in the same condition.
{{< /drg/did-you-know >}}

### When Cleaning Is Appropriate

Some items do benefit from careful cleaning — a fossil still embedded in rock matrix, a mineral caked with clay, or a vintage toy covered in surface dust. The key is research: learn the accepted cleaning methods for your specific type of collection before you touch anything.

## Storage

Good storage protects against five enemies of collectibles:

1. **Moisture** — causes rust, mold, foxing (brown spots on paper), and mineral degradation
2. **Light** — fades colors, yellows paper, and degrades plastics
3. **Temperature extremes** — expansion and contraction crack, warp, and delaminate
4. **Dust and pollutants** — abrade surfaces and cause chemical reactions
5. **Pests** — insects and rodents can destroy paper, fabric, and natural specimens

### Storage Solutions by Collection Type

- **Coins:** Acid-free flips, capsules, or slabs (hard plastic holders used by grading services)
- **Stamps:** Stamp mounts (hingeless) in acid-free albums
- **Cards:** Penny sleeves inside top-loaders or magnetic holders, stored upright in boxes
- **Rocks & minerals:** Padded compartment boxes or display cases with individual wells
- **Paper items:** Acid-free sleeves, flat storage in archival boxes, away from light

{{< drg/checklist title="Storage Environment Checklist" subtitle="Keep your collection safe from environmental damage" >}}
- Stable temperature: Aim for 65–72°F with minimal fluctuation.
- Low humidity: 30–50% relative humidity is ideal for most collections.
- No direct sunlight: UV light is the top cause of fading and deterioration.
- Clean air: Avoid attics, basements, and garages where dust, moisture, and temperature swing.
- Pest prevention: Keep food away from storage areas and inspect regularly.
{{< /drg/checklist >}}

{{< drg/image src="images/preservation-supplies.avif" alt="An array of archival preservation supplies laid out on a table: acid-free album pages, cotton gloves, stamp tongs, coin capsules, penny sleeves, top-loaders, and a soft felt pad" >}}

{{< drg/external-link
    title="American Institute for Conservation — Caring for Your Treasures"
    url="https://www.culturalheritage.org/resources/caring-for-your-treasures"
    description="Expert preservation advice from professional conservators, organized by material type." >}}

{{< drg/image src="images/storage-environment-comparison.avif" alt="Split comparison: left side shows a humid attic with fluctuating temperatures and sunlight damage on stored items; right side shows a climate-controlled room with archival boxes on shelves and a hygrometer showing ideal humidity" >}}

{{< drg/next-page
    text="Now that you know how to protect your collection from damage"
    teaser="learn how to show it off with a great display."
    url="/merit-badges/collections/guide/req3b/" >}}
