---
title: "Safety in Service and Lifestyle"
layout: "guide"
group_title: "9. Careers and Service"
req_number: "9b"
req_path: "9.b"
prev: "/merit-badges/safety/guide/req9a/"
prev_title: "Safety Careers"
next: "/merit-badges/safety/guide/extended-learning/"
next_title: "Extended Learning"
---

{{< drg/requirement number="9b" >}}
Explore how you could use knowledge and skills related to safety to pursue a hobby, lifestyle, or service as a volunteer. Research any training needed, expenses, and organizations that promote or support it. Discuss with your counselor what short-term and long-term goals you might have if you pursue this.
{{< /drg/requirement >}}

Safety skills are not only for professionals. They also matter to people who volunteer, lead outdoor groups, coach youth, patrol trails, assist at events, or make preparedness part of daily life. This option asks you to picture what safety looks like when it becomes part of who you are, not just what job you do.

### Possible directions to explore

You might research paths such as:

- volunteer search and rescue support
- community emergency response teams (CERT)
- park or trail volunteering
- camp health and safety support roles
- event safety volunteering
- outdoor leadership with extra training in first aid, weather, and risk management

### What to research

Even volunteer and hobby paths still require preparation. Look into:

- training or certifications you would need
- equipment or uniform costs
- time commitment
- what organizations support the activity
- what beginners can realistically do first
- what long-term involvement could look like

### Turn interest into goals

This requirement becomes stronger when you separate short-term and long-term goals.

- **Short-term goals** might include taking first-aid training, joining a local service group, attending a preparedness workshop, or volunteering at a park event.
- **Long-term goals** might include becoming a wilderness trip leader, joining mountain rescue support, serving in community emergency planning, or building a lifestyle centered on preparedness and helping others.

## Official Resources

{{< drg/video
    title="Volunteers in Mountain Rescue (video)"
    url="https://youtu.be/SfFb2D-3H2s" >}}

{{< drg/video
    title="Safety Tips for Serving as Park Volunteer (video)"
    url="https://youtu.be/ZlgAJHEWmeg?si=-sKGTI3ErflGOuA_" >}}

{{< drg/external-link
    title="Safety as a Passion (website)"
    url="https://www.iloveit.net/passion/safety-as-a-passion-and-hobby/"
    description="Shows how safety thinking can grow into a long-term interest, service mindset, or practical hobby." >}}

{{< drg/be-prepared title="Turning interest into action" >}}

- **Pick one path** that sounds real to you.
- **Find the beginner step** such as a course, meeting, or volunteer orientation.
- **Estimate the cost and time** so your plan is realistic.
- **Tell your counselor how you would start this year, not just someday.**
  {{< /drg/be-prepared >}}

By this point, you have explored safety as a skill, a habit, a community responsibility, and a possible future path. The Extended Learning page will take you beyond the requirements into how safety systems are designed and improved in the real world.

{{< drg/next-page
    text="You have finished the final requirement option."
    teaser="Go beyond the badge with deeper ideas about safety culture, near misses, and real-world preparedness."
    url="/merit-badges/safety/guide/extended-learning/" >}}