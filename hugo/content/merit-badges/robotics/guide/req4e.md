...
{{< drg/checklist title="Useful test notes" subtitle="Write down enough detail to learn from each run" >}}
...
{{< drg/be-prepared title="The robot works sometimes, but not every time" >}}
If your robot succeeds on one run and fails on the next:

- **Check consistency first.** Look for loose parts, weak battery charge, or changing starting positions.
- **Simplify the test.** Remove extra variables and test one behavior at a time.
- **Review sensor placement and thresholds.** Inconsistent input often causes inconsistent output.
- **Document the pattern.** Random-looking problems often become clearer when written down. 
{{< /drg/be-prepared >}}

{{< drg/tip >}}
Take pictures from more than one angle before you start changing the robot again. Those photos help you explain the finished version even if the build evolves later.
{{< /drg/tip >}}
...
{{< drg/next-page
    text="You have tested the robot and gathered real evidence"
    teaser="Next, demonstrate the robot and explain how the design process went."
