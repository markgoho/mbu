---
title: "Req 6f — Cleaning a Drain Trap"
layout: "guide"
group_title: "Hands-On Plumbing Practice"
req_number: "6f"
req_path: "6.f"
prev: "/merit-badges/plumbing/guide/req6e/"
prev_title: "Req 6e — Replacing a Faucet"
next: "/merit-badges/plumbing/guide/req6g/"
next_title: "Req 6g — Applying Thread Tape"
---

{{< drg/requirement number="6f" >}}
Remove, clean or replace, and reinstall a sink or lavatory drain trap.
{{< /drg/requirement >}}

A drain trap is one of the simplest plumbing parts to see and understand, which makes it a great learning project. It is also one of the first places where hair, soap scum, or debris can cause trouble.

{{< drg/video
    title="How To Install P-Trap And Bathroom Sink Drain Piping | Easy Installation For Beginners! (video)"
    url="https://youtu.be/OFui97xhln0?si=0j-wNeNigMTLVJHt" >}}

## Why the trap matters

The trap holds water that blocks sewer gases from rising into the room. That means the trap is doing two jobs at once: carrying wastewater away and maintaining a protective water seal.

## Basic trap service steps

1. Put a bucket or pan underneath.
2. Loosen the slip nuts carefully.
3. Lower the trap and expect dirty water.
4. Clean out debris or replace worn parts.
5. Reassemble the trap in the correct orientation.
6. Run water and inspect every connection for leaks.

## What to look for during the job

- Cracks in plastic parts
- Corrosion on older metal pieces
- Misalignment that puts stress on the joints
- Worn washers or sealing surfaces
- Debris that explains the slow drain or odor

{{< drg/tip >}}
Lay the removed parts on a towel in the order they came apart. That makes it much easier to remember how the washers and nuts go back together.
{{< /drg/tip >}}

{{< drg/checklist title="Trap Reassembly Check" subtitle="Before you call the job finished" >}}

- **All nuts are hand-started correctly**
- **Washers face the correct direction**
- **The trap lines up naturally without forcing**
- **Water runs without drips**
- **No sewer odor appears after the trap refills**
  {{< /drg/checklist >}}

{{< drg/safety-first >}}
Trap water can smell bad and may contain bacteria. Wear gloves if appropriate, avoid splashing, and clean the area afterward. If the trap is metal and badly corroded, do not force it with too much pressure.
{{< /drg/safety-first >}}

{{< drg/image src="images/p-trap-cleanout-sequence.avif" alt="Step-by-step sink trap removal and reinstallation with bucket, slip nuts, washers, and cleaned trap pieces labeled" >}}

This repair teaches how drainage parts fit together. The next task is smaller, but it rewards the same careful attention to detail: applying thread tape correctly.

{{< drg/next-page
    text="You have followed the full sequence for taking apart, cleaning, and reinstalling a trap."
    teaser="Next, finish the hands-on options with the small but important skill of applying thread tape correctly."
    url="/merit-badges/plumbing/guide/req6g/" >}}