---
title: "Req 2f — Replacing a Window Pane"
layout: guide
group_title: "Outdoor Repairs"
req_number: "2f"
prev: "/merit-badges/home-repairs/guide/req2e/"
prev_title: "Req 2e — Screen Repair"
next: "/merit-badges/home-repairs/guide/req3/"
next_title: "Req 3 — Shop and Structure Projects"
---

{{< drg/requirement number="2f" >}}
Replace a pane of glass.
{{< /drg/requirement >}}

Broken glass changes the tone of a repair job immediately. What might look like a simple replacement becomes a safety-first project because sharp fragments, old glazing, and exposed window openings can all cause injuries if you rush.

## Start With Safety and Supervision

This requirement must be done carefully and with close adult supervision. Before any new glass goes in, the broken pane and surrounding frame need to be made safe.

{{< drg/safety-first >}}
Wear eye protection, sturdy gloves, long sleeves, and closed-toe shoes when handling broken glass. Keep bystanders away from the work area and collect every fragment before moving on. Never brush glass aside with bare hands.
{{< /drg/safety-first >}}

## What the Job Usually Involves

The exact steps vary by window type, but the basic process often includes:

1. Removing loose broken glass safely
2. Cleaning old glazing material or other holding components from the frame
3. Measuring the opening accurately
4. Fitting the replacement pane
5. Securing it according to the window design
6. Cleaning up every piece of debris afterward

{{< drg/video
    title="How to Replace a Broken Window Pane (video)"
    url="https://youtu.be/Dc67opK57sk?si=xnMQ0ueGHn6NUdtN" >}}

This is a good resource because it shows how much of the work happens before the new glass is even touched. Careful cleanup, measurement, and frame preparation matter just as much as the final installation.

{{< drg/image src="images/window-pane-replacement-ppe.avif" alt="Safe broken-window repair setup with protective gear, contained glass cleanup, and measured replacement pane" >}}

## Why Measuring Matters So Much

A pane that is too large may not fit safely. A pane that is too small may rattle, leave gaps, or sit poorly in the frame. Window repairs reward accuracy.

That makes this requirement a good reminder that home repair is not only about force or tools. Sometimes the most important skills are patience, observation, and clean technique.

{{< drg/be-prepared title="If the Frame Itself Is Damaged" >}}
If you remove the broken glass and discover rotten wood, cracked frame material, or badly bent metal:

- Pause the project and show the damage to your supervising adult.
- Decide whether the pane replacement is still the right repair.
- Be ready to explain that glass replacement may depend on the condition of the frame holding it.
{{< /drg/be-prepared >}}

## What Your Counselor Should See

Your counselor will not just be looking for “new glass installed.” They will be looking for whether you handled a fragile and hazardous material responsibly.

You should be able to explain:

- How you kept the area safe during removal
- How you measured or checked the replacement size
- How you cleaned and prepared the frame
- How you disposed of broken glass safely
- What supervision was especially important for this task

After this final outdoor option, the guide shifts into workshop and structure repairs. Those next projects will still require care, but they trade broken glass and weather exposure for measuring, fastening, support, and finish work.

{{< drg/next-page
    text="You now understand why replacing a pane of glass depends on careful removal, accurate measuring, and complete cleanup."
    teaser="Next, choose three workshop and structure projects such as tool storage, workbench building, furniture repair, and fence work."
    url="/merit-badges/home-repairs/guide/req3/" >}}
