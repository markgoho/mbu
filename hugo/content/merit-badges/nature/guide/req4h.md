---
title: "Req 4h — Soils and Rocks"
layout: "guide"
group_title: "Earth Materials"
req_number: "4h"
req_path: "4.h"
prev: "/merit-badges/nature/guide/req4g2b/"
prev_title: "Req 4g2b — Build a Photo Catalog"
next: "/merit-badges/nature/guide/req5/"
next_title: "Req 5 — Outdoor Ethics in Nature Study"
---

{{< drg/requirement number="4h" >}}
Soils and Rocks. Do ALL of the following:
{{< /drg/requirement >}}

Nature study is not only about living things. Soil and rock shape which plants can grow, how water moves, and what habitats develop. This option helps you read the ground beneath your feet.

## Requirement 4h1

{{< drg/inherited-requirement number="4h1" req_path="4.h.1" topic="Collect and identify three different types of soil that represent soils high in sand, clay and humus." />}}

### Recognizing sandy soil

Sandy soil feels gritty. Water drains through it quickly, and it does not hold together well in your hand.

### Recognizing clay-rich soil

Clay feels smooth and sticky when wet. It holds water longer and can be shaped or rolled more easily than sandy soil.

### Recognizing humus-rich soil

Humus-rich soil is dark, crumbly, and full of decomposed organic matter. It often has an earthy smell and supports lots of life.

{{< drg/video
    title="Layers of Soil | Soil Profile Formation (video)"
    url="https://www.youtube.com/watch?v=VTuL5Pa0gC0" >}}

{{< drg/checklist title="Simple soil comparison clues" subtitle="What to note when you collect samples" >}}
- Color
- Texture
- How well it holds together
- How quickly water drains
- Organic matter such as roots or decomposed leaves
{{< /drg/checklist >}}

## Requirement 4h2

{{< drg/inherited-requirement number="4h2" req_path="4.h.2" topic="Collect and identify five different types of rocks from your area." />}}

### Look at texture and grain

Some rocks have visible grains. Others are smooth and dense. Some break into layers, and some have crystals or holes. These details help you sort rocks into groups like igneous, sedimentary, and metamorphic.

### Keep location with the rock

Where you found the rock matters. Stream rocks are often rounded by water. Road cuts may reveal local layers. Gravel piles may contain stones brought from somewhere else, so local natural sites usually make better evidence.

### Use plain language if needed

If you cannot identify a rock to a very specific name, begin with what you can observe: color, grain size, hardness, layering, sparkle, or rounded edges. Careful observation is still part of good nature study.

{{< drg/video
    title="Rockhounding 101: What to Look for on Your Next Adventure! (video)"
    url="https://youtu.be/yORP1Ba-K4Q?si=iR3Lo66iW-dmFjno" >}}

{{< drg/video
    title="Rock & Mineral Identification | It Can Be Easier Than You Think (video)"
    url="https://www.youtube.com/watch?v=mgld5jeYR04" >}}

{{< drg/tip >}}
Use small bags or paper envelopes and label them immediately. Soil and rock samples become confusing fast when several look similar after you get home.
{{< /drg/tip >}}

{{< drg/image src="images/soil-and-rock-comparison-board.avif" alt="Field study board showing labeled samples of sandy soil, clay soil, humus-rich soil, and five local rocks with texture notes" >}}

You have now finished the menu of field-study options. The next requirement turns from observation to outdoor ethics and how Scouts should behave in natural areas.

{{< drg/next-page
    text="You have compared earth materials that shape habitats and ecosystems."
    teaser="Next, connect all your field work to Leave No Trace and the Outdoor Code."
    url="/merit-badges/nature/guide/req5/" >}}
