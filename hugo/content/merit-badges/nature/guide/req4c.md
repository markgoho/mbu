---
title: "Reptiles and Amphibians"
layout: "guide"
group_title: "4. Herps in the Field"
req_number: "4c"
req_path: "4.c"
prev: "/merit-badges/nature/guide/req4b/"
prev_title: "Mammal Signs and Tracks"
next: "/merit-badges/nature/guide/req4d/"
next_title: "Insects and Spiders Up Close"
---

{{< drg/requirement number="4c" >}}
Reptiles and Amphibians. Do ALL of the following:
{{< /drg/requirement >}}

Reptiles and amphibians are easy to overlook because many stay hidden, blend into their surroundings, or appear only in the right weather. This option teaches you to identify them safely by sight, sound, and field signs.

## Requirement 4c1

{{< drg/requirement number="4c1" >}}
Show that you can recognize the venomous snakes in your area.
{{< /drg/requirement >}}

Start with your own state or region. Do not try to memorize every venomous snake in North America. Learn the species that actually live near you, where they are found, and what markings, head shape, body pattern, and habitat clues can help you recognize them.

A strong answer includes more than “this one is dangerous.” Be ready to explain how you would avoid disturbing it, back away slowly, and leave it alone.

{{< drg/external-link
    title="How to Tell if a Snake Is Venomous (website)"
    url="https://urbanjunglewildliferemoval.com/blog/how-to-tell-venomous-snake/"
    description="A starter overview of features people use when learning to recognize venomous snakes, along with caution about not relying on myths." >}}

{{< drg/video
    title="Zoologist Explains Snake Venom - Poisonous vs Venomous (video)"
    url="https://youtu.be/KxXfWrI6bB4?si=6TMzt9gUfQdxCvWc" >}}

{{< drg/safety-first >}}
Never try to catch, pin, move, or pose a snake for a photo. Recognition means identifying it safely from a distance and knowing how to avoid a bite.
{{< /drg/safety-first >}}

## Requirement 4c2

{{< drg/inherited-requirement number="4c2" req_path="4.c.2" topic="In the field, identify three species of reptiles or amphibians." />}}

### Where to look for reptiles and amphibians

Turtles may bask on logs. Frogs gather near shallow water. Salamanders often stay under logs or rocks in damp places. Lizards may sun themselves on warm surfaces. Habitat is often your first clue.

### What details help with identification

Look at body shape, skin texture, color pattern, tail shape, toes, and where the animal was found. Reptiles usually have dry scales. Amphibians usually have moist skin and stay closer to water or damp cover.

{{< drg/external-link
    title="Reptiles of the United States (website)"
    url="https://www.inaturalist.org/places/united-states#taxon=26036"
    description="A searchable community-science reference that helps you compare reptiles by range and appearance." >}}

{{< drg/external-link
    title="Amphibians of the United States (website)"
    url="https://www.inaturalist.org/places/united-states#taxon=20978"
    description="A field-friendly way to compare amphibians reported in different regions and habitats." >}}

## Requirement 4c3

{{< drg/inherited-requirement number="4c3" req_path="4.c.3" topic="Recognize one species of toad or frog by voice; OR identify one reptile or amphibian by eggs, den, burrow, or other signs." />}}

### Recognizing frogs and toads by voice

Spring evenings can sound like a giant orchestra of frogs. Focus on one common species first. Learn whether its call is a peep, trill, croak, or clucking sound, and connect that sound to wetland type and season.

### Identifying reptiles or amphibians by signs

If you choose the sign option, look for egg masses in shallow water, turtle nests in sandy soil, snake sheds, basking spots, or burrows associated with known species. A good field note explains why you think the sign belongs to that animal.

{{< drg/video
    title="Reptiles vs. Amphibians (video)"
    url="https://www.youtube.com/watch?v=6MsTbQi19SA" >}}

{{< drg/video
    title="Frog Sounds: Why They Make Them (Examples) (video)"
    url="https://youtu.be/74-vaZv54s8?si=7ClcX_Akwhjh0A_T" >}}

{{< drg/tip >}}
Audio recordings help. If you can legally and safely record a frog call on your phone, compare it later with a trusted source and note the date, temperature, and habitat.
{{< /drg/tip >}}

{{< drg/image src="images/frog-call-and-egg-mass-guide.avif" alt="Educational diagram showing a calling frog in wetland habitat and examples of frog egg masses and snake shed clues" >}}

This option sharpens your field senses. Next, move even smaller and study the busy world of insects and spiders.

{{< drg/next-page
    text="You have practiced safe reptile and amphibian observation by sight, sound, and sign."
    teaser="Next, look closely at insects and spiders through identification and life cycles."
    url="/merit-badges/nature/guide/req4d/" >}}
