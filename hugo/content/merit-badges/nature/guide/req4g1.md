---
title: "Req 4g1 — Wild Plant Identification"
layout: "guide"
group_title: "Plant Study"
req_number: "4g1"
req_path: "4.g.1"
prev: "/merit-badges/nature/guide/req4f/"
prev_title: "Req 4f — Shells, Mollusks, and Crustaceans"
next: "/merit-badges/nature/guide/req4g2/"
next_title: "Req 4g2 — Seeds, Leaves, and Plant Records"
---

{{< drg/requirement number="4g1" >}}
In the field, identify 15 species of wild plants.
{{< /drg/requirement >}}

This requirement rewards slow looking. Plants do not run away, but they can still be tricky because many change with the season, look different when young, or resemble close relatives.

### Start with the whole plant

Before zooming in on tiny details, ask broad questions. Is it a tree, shrub, vine, grass, fern, or flowering plant? Is it growing in sun or shade? Dry soil or wet soil? Field edge or forest interior?

### Look for reliable clues

Leaves are useful, but not enough by themselves. Notice whether leaves are opposite or alternate on the stem, whether their edges are smooth or toothed, and whether the plant has flowers, fruit, cones, thorns, or a distinctive smell.

### Use habitat as a clue

Some plants strongly prefer certain places. Cattails suggest shallow water. Pines often dominate dry, sunny ground. Mosses point to moisture and shade. Habitat does not prove identification by itself, but it helps you narrow the possibilities.

{{< drg/video
    title="How to Identify Wild Plants - A Guide To Botanical Terms (video)"
    url="https://www.youtube.com/watch?v=5Aj82u2he6s" >}}

{{< drg/checklist title="What to record for each plant" subtitle="Useful details for your notes" >}}
- Date and location
- Habitat type
- Growth form: tree, shrub, vine, forb, grass, fern, or other
- Leaf shape and arrangement
- Flowers, fruit, seeds, or bark if present
{{< /drg/checklist >}}

{{< drg/safety-first >}}
Do not taste unknown plants, and avoid handling anything that could be poisonous or cause a rash. Learn poison ivy, poison oak, or poison sumac for your area before you begin serious field work.
{{< /drg/safety-first >}}

{{< drg/tip >}}
Fifteen species is easier if you spread the work across two or three outings in different habitats. A wetland edge, a sunny field, and a woodland trail can produce very different plant lists.
{{< /drg/tip >}}

{{< drg/image src="images/wild-plant-identification-clues.avif" alt="Wild plant study diagram showing leaf arrangement, flower shape, bark, and habitat notes in a field notebook" >}}

Once you can identify plants in the field, the next step is to organize plant evidence by either collecting labeled specimens or creating a photo catalog.

{{< drg/next-page
    text="You have practiced identifying wild plants with field clues and habitat notes."
    teaser="Next, choose whether to build a labeled specimen set or a photo catalog."
    url="/merit-badges/nature/guide/req4g2/" >}}
