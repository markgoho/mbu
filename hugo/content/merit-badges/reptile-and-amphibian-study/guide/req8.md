---
title: "Choose Your Observation Project"
layout: "guide"
group_title: "8. Hands-On Observation"
req_number: "8"
req_path: "8"
prev: "/merit-badges/reptile-and-amphibian-study/guide/req7/"
prev_title: "Venomous Species and Bite Response"
next: "/merit-badges/reptile-and-amphibian-study/guide/req8a/"
next_title: "Care for a Captive Animal"
---

{{< drg/requirement number="8" >}}
Do ONE of the following:
{{< /drg/requirement >}}

You complete exactly **one** option here. Both options teach close observation, careful recordkeeping, and responsible animal care, but they fit different situations.

## Your Options

**[Req 8a — Care for a Captive Animal](/merit-badges/reptile-and-amphibian-study/guide/req8a/)**: Care for one or more reptiles or amphibians, or their eggs or larvae, for at least a month with your counselor's approval. This option is best if you already have access to an approved animal and can manage daily care responsibly. You will gain hands-on experience with habitat setup, feeding, humidity, temperature, and long-term observation.

**[Req 8b — Study an Animal in Captivity](/merit-badges/reptile-and-amphibian-study/guide/req8b/)**: Visit the same reptile or amphibian weekly for three months at a zoo, aquarium, classroom, rescue, or nature center. This option is best if you cannot keep an animal yourself but can return to the same specimen regularly. You will gain strong observation skills and learn from professional caretakers.

## How to Choose

{{< drg/checklist title="Choosing Between 8a and 8b" subtitle="Pick the option you can do well from start to finish" >}}
- **Daily responsibility**: 8a requires regular direct care. 8b requires weekly visits instead.
- **Access**: 8a depends on counselor-approved animal care. 8b depends on access to a stable exhibit or program.
- **What you will learn**: 8a teaches hands-on husbandry. 8b teaches long-term observation and interviewing caretakers.
- **Time pattern**: 8a is shorter but more frequent. 8b lasts longer but usually involves fewer visits.
{{< /drg/checklist >}}

{{< drg/tip >}}
If you are new to reptile and amphibian care, 8b is often the simpler and safer option because professionals are already managing the animal's needs. Choose 8a only if you can provide steady, approved care for the entire period.
{{< /drg/tip >}}

## What Both Options Have in Common

Whichever route you choose, your counselor will expect more than a few quick notes. You need to show how the animal lives, what changed over time, and how you paid attention to the habitat and care needs.

Bring records that include:

- dates of observations
- what the animal ate or refused
- behavior changes
- coloration or shedding changes
- habitat conditions such as light, humidity, and temperature
- questions you asked and what you learned

{{< drg/video
    title="The Most VENOMOUS Snakes in the US (video)"
    url="https://youtu.be/guMzO7vzMT0?si=3thQ8FQdwoEHXV75" >}}

Even though this official resource is about venomous snakes rather than captive care, it is a useful reminder that some reptiles must only be handled or housed by experts. For this badge, choose safe, approved animals and settings.

{{< drg/safety-first >}}
Never collect a wild animal on your own just because you want something to observe. Follow your counselor's approval, local laws, and the welfare needs of the species first.
{{< /drg/safety-first >}}

{{< drg/next-page
    text="You know the difference between the two project paths and what each one will require from you."
    teaser="Start with Option 8a if you will be directly caring for an approved reptile, amphibian, egg, or larva."
    url="/merit-badges/reptile-and-amphibian-study/guide/req8a/" >}}
