---
title: "Req 7 — Towing a Swimmer"
layout: "guide"
group_title: "Rescue Skills"
req_number: "7"
req_path: "7"
prev: "/merit-badges/rowing/guide/req6/"
prev_title: "Req 6 — Swamped Boat Recovery"
next: "/merit-badges/rowing/guide/req8/"
next_title: "Req 8 — Craft, Materials & Oarlocks"
---

{{< drg/requirement number="7" >}}
Alone in a rowboat, push off from the shore or a pier. Row 20 yards to a swimmer. While giving instructions to the swimmer, pivot the boat so that the swimmer can hold on to the stern. Tow the swimmer to shore.
{{< /drg/requirement >}}

A swimmer rescue from a rowboat is a control problem before it is a strength problem. If you rush the approach, let the swimmer grab the wrong part of the boat, or forget to talk, the rescue can become less stable for both of you.

The stern tow works because it keeps the swimmer away from the oars and lets the rower keep better control of the boat.

## Push Off and Approach the Swimmer

Launch cleanly and row toward the swimmer with enough speed to reach them, but not so much that you overshoot or create confusion. Keep watching the swimmer's condition. A calm swimmer can follow directions. A panicked swimmer may need very short, clear commands.

## Give Instructions Early

Do not wait until the swimmer is already grabbing the hull. Tell them what you want before they touch the boat.

Useful instructions are short:

- "Stay calm."
- "I am turning the stern toward you."
- "Hold the stern, not the sides."
- "Keep your body behind the boat."

## Pivot the Boat for the Stern Hold

The pivot is the heart of the requirement. You are positioning the safest part of the boat for contact. The stern gives the swimmer a stable point without interfering with your oars.

Once the swimmer has the stern, check that they are actually secure before you start towing.

{{< drg/checklist title="Swimmer-Tow Sequence" subtitle="Do the rescue in a calm order" >}}
- **Approach under control.**
- **Talk before contact.**
- **Turn the stern to the swimmer.**
- **Confirm a safe hold.**
- **Tow steadily to shore.**
- **Avoid sudden changes that could shake the swimmer loose.**
{{< /drg/checklist >}}

## Tow to Shore

A tow is easier when you stay smooth. Sudden power changes can make the swimmer lose grip or swallow water. Choose the shortest safe route to shore, but do not head through obstacles, traffic, or breaking waves if there is a safer angle.

This requirement also reminds you why [Req 2](/merit-badges/rowing/guide/req2/) matters. Rescue work is tiring. The rower needs enough swimming and water confidence to stay calm while the boat is only partly under ideal control.

{{< drg/be-prepared title="The swimmer is scared and tries to grab the side of the boat" >}}
- **Use your voice first.** Tell them to move to the stern.
- **Keep the boat stable.** Do not jerk away in a way that flips the problem into panic.
- **Repeat one simple instruction.** Too many words are hard to follow in stress.
- **Protect the oars and your control.** The stern setup exists for a reason.
{{< /drg/be-prepared >}}

{{< drg/safety-first >}}
This is a practiced rescue skill, not a stunt. Always train it with qualified supervision and a designated swimmer who knows the drill.
{{< /drg/safety-first >}}

{{< drg/image src="images/stern-tow-rowboat.avif" alt="Top-down rescue diagram showing a rowboat pivoted so a swimmer holds the stern for a tow to shore" >}}

A good rescue rower does not just reach the swimmer. They keep the whole situation organized. Next, the guide shifts from rescue skills to rowing knowledge about craft types, materials, and oarlocks.

{{< drg/next-page
    text="You now understand how to reach a swimmer, position the boat safely, and tow them to shore."
    teaser="Next, learn the equipment side of rowing: different craft, building materials, and oarlock designs."
    url="/merit-badges/rowing/guide/req8/" >}}
