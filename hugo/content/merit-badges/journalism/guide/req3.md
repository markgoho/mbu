---
title: "Choose a Storytelling Challenge"
layout: guide
group_title: "3. Storytelling and Interviews"
req_number: "3"
prev: "/merit-badges/journalism/guide/req2b2/"
prev_title: "Visiting a Station"
next: "/merit-badges/journalism/guide/req3a/"
next_title: "Write the Story"
---

{{< drg/requirement number="3" >}}
Discuss the differences between a hard news story and a feature story. Explain what the five W's and H are. Then do ONE of the following:
{{< /drg/requirement >}}

This requirement has two parts before you make your choice:

- understand the difference between a **hard news story** and a **feature story**
- explain the **five W's and H**: who, what, when, where, why, and how

## Hard News vs. Feature Stories

A **hard news story** tells readers what happened and why it matters right now. It usually covers something timely: an election result, accident, weather emergency, meeting decision, or major announcement. Hard news tends to get to the point fast.

A **feature story** still reports facts, but it slows down and explores people, scenes, emotions, or background in more depth. A feature might profile a volunteer, explain a tradition, or show how an event affected people over time.

Think of it this way: hard news answers, "What happened today?" A feature answers, "What does this mean, and what is it like?"

## The Five W's and H

These questions help you gather enough information for a trustworthy story:

- **Who** was involved?
- **What** happened?
- **When** did it happen?
- **Where** did it happen?
- **Why** did it happen or matter?
- **How** did it happen?

{{< drg/image src="images/five-ws-and-h-reporting-wheel.avif" alt="Illustrated wheel showing who, what, when, where, why, and how around a reporter's notebook" >}}

If you cannot answer several of these clearly, your reporting probably needs more work.

{{< drg/image src="images/hard-news-vs-feature-structure.avif" alt="Split comparison of hard news structure versus feature story structure with different reading flow" >}}

## Your Four Choices

**[Req 3a — Write the Story](/merit-badges/journalism/guide/req3a/)**: Write either a hard news or feature article about a current or unusual event.

**[Req 3b — Interview an Influential Person](/merit-badges/journalism/guide/req3b/)**: Interview someone important in your community and report what you learned.

**[Req 3c — Profile a Journalist](/merit-badges/journalism/guide/req3c/)**: Read a journalist's autobiography and write about that person's life and contributions.

**[Req 3d — Cover a Scouting Event](/merit-badges/journalism/guide/req3d/)**: Attend a Scouting event, write a 200-word article, and submit it for consideration.

{{< drg/checklist title="Choosing Your Storytelling Challenge" subtitle="Pick the option that best fits your interests and access" >}}

- **Want to cover something happening now?** Choose 3a.
- **Enjoy asking questions face to face?** Choose 3b.
- **Like reading about interesting careers and history?** Choose 3c.
- **Already have a troop, district, or council event coming up?** Choose 3d.
- **What you will gain:** 3a builds reporting structure, 3b strengthens interviewing, 3c teaches research and biography writing, and 3d adds real submission experience.
{{< /drg/checklist >}}

{{< drg/tip >}}
If you are deciding between two options, choose the one that gives you access to the strongest reporting material. A great interview subject or a timely event usually leads to a better story.
{{< /drg/tip >}}

{{< drg/video
    title="Hard News vs. Soft News: News Types Explained — K20 Center"
    url="https://www.youtube.com/watch?v=-7lVkYt4jjw" >}}

Next, start with the first choice: writing a news or feature story from a current or unusual event.

{{< drg/next-page
    text="You understand the difference between hard news and feature writing, and you know the five W's and H."
    teaser="Next, learn how to choose an event and shape it into a strong story."
    url="/merit-badges/journalism/guide/req3a/" >}}
