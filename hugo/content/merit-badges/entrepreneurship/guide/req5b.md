---
title: "Market Analysis"
layout: guide
group_title: "5. Building Your Business Plan"
req_number: "5b"
prev: "/merit-badges/entrepreneurship/guide/req5a/"
prev_title: "Product or Service"
next: "/merit-badges/entrepreneurship/guide/req5c/"
next_title: "Financial Plan"
---

{{< drg/requirement number="5b" >}}
Market Analysis
{{< /drg/requirement >}}

This section of your business plan forces you to step outside your own enthusiasm and look at your idea through the eyes of the market. You might love your product, but the real question is: **Will other people pay for it?**

Requirement 5b covers three sub-topics:

- **Target customers** — who is going to buy?
- **Competitors** — who else is selling something similar?
- **Unique value** — why would customers choose you?

### Identifying Your Target Customers

Your **target market** is the specific group of people most likely to buy your product or service. "Everyone" is never the right answer. Even the biggest companies in the world focus on specific customer segments. The more clearly you define your target customer, the better you can design, price, and market your offering.

To define your target market, answer these questions:

- **Age range:** Are your customers kids, teens, adults, or seniors?
- **Location:** Are they in your neighborhood, your town, or online?
- **Need:** What problem are you solving for them? Why would they care?
- **Budget:** How much can they afford to spend?
- **Habits:** Where do they already shop? How do they find new products?

{{< drg/tip >}}
Try creating a **customer persona** — a fictional profile of your ideal customer. Give them a name, age, and backstory. For example: "Maria is a busy parent with two kids under 10. She hosts birthday parties every few months and does not have time to bake custom treats. She would pay $25–$40 for a dozen custom-decorated cookies delivered to her door." This persona helps you make every business decision through the lens of a real person.
{{< /drg/tip >}}

Here is how to research whether your target customers actually exist in your area:

1. **Talk to people.** Ask friends, family, neighbors, and classmates if they would buy your product and what they would pay.
2. **Observe.** Visit local shops, browse online marketplaces, and see who is buying similar products.
3. **Count.** Estimate how many potential customers are within your reach. A lawn care business in a neighborhood of 200 homes has a clear, measurable market.
4. **Survey.** Create a simple survey (Google Forms is free) and share it with your target audience.

### Analyzing Your Competitors

A competitor is anyone who offers a product or service that your target customers might buy instead of yours. Identifying your competitors is not about being scared of them — it is about understanding the landscape so you can position yourself strategically.

For each competitor, document:

| Factor | What to Assess |
|--------|---------------|
| **Who they are** | Name, location, size |
| **What they offer** | Products/services, quality level |
| **Pricing** | How much they charge |
| **Strengths** | What they do well — reputation, convenience, variety |
| **Weaknesses** | Where they fall short — slow delivery, limited options, poor customer service |

{{< drg/image src="images/competitor-analysis-notebook.avif" alt="A notebook page showing a handwritten competitor analysis grid with three columns for different businesses, rows for strengths, weaknesses, pricing, and customer reviews" >}}

{{< drg/did-you-know >}}
When Netflix started in 1997, its founder Reed Hastings studied Blockbuster's weaknesses — late fees, limited selection, and the inconvenience of driving to a store. He built Netflix specifically to address those pain points. Understanding your competitors' weaknesses is how you find your competitive advantage.
{{< /drg/did-you-know >}}

**Where to find competitors:**

- Search Google for your product or service in your area
- Check local business directories and social media
- Ask potential customers who they currently buy from
- Visit farmers' markets, craft fairs, or local shops
- Search Etsy, Amazon, or other online platforms

### Describing Your Unique Value

Your **unique value proposition (UVP)** answers the question: **Why should a customer choose you over the competition?** This is the most important sentence in your market analysis — possibly in your entire business plan.

Your UVP might be based on:

- **Price:** You charge less than competitors (but be careful — competing on price alone is risky)
- **Quality:** Your product is better made, fresher, or more durable
- **Convenience:** You deliver, come to the customer, or offer faster service
- **Customization:** You offer options competitors do not
- **Personal connection:** You know your customers and their preferences
- **Niche focus:** You serve a specific audience that big competitors ignore

{{< drg/be-prepared title="Testing Your Unique Value" >}}
Here is a quick test for your UVP. Fill in this sentence:

**"My business offers __________ for __________ who need __________, unlike __________ because __________."**

Example: "My business offers custom-decorated sugar cookies for busy parents who need impressive party treats without the hassle, unlike the grocery store bakery because every order is personalized and delivered to the door."

If you cannot fill in all five blanks, your unique value needs more thought.
{{< /drg/be-prepared >}}

### Putting It All Together

Your market analysis section in the business plan should flow logically:

1. **Define your target customer** with specific detail
2. **Estimate market size** — how many potential customers are within reach
3. **List 2–3 competitors** with honest assessments of their strengths and weaknesses
4. **State your unique value proposition** clearly and confidently
5. **Explain why customers will switch to you** (or try you for the first time)

This section shows your counselor — and any future investor — that your idea is not based on wishful thinking. You have done the research, you understand the competition, and you have a clear reason why customers will choose you.

{{< drg/external-link
    title="Census Bureau QuickFacts"
    url="https://www.census.gov/quickfacts/"
    description="Look up population demographics for your city or county — useful for estimating your market size and understanding your local customer base." >}}

{{< drg/next-page
    text="You know your customers and your competition"
    teaser="Now it is time to talk numbers — how much will it cost, and how will you make money?"
    url="/merit-badges/entrepreneurship/guide/req5c/" >}}
