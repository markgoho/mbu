---
title: "Introduction & Overview"
layout: guide
badge_name: "Lifesaving"
group_title: "Getting Started"
next: "/merit-badges/lifesaving/guide/req1/"
next_title: "Safe Swim Defense"
guide_nav:
  - group_title: "Getting Started"
    items:
      - title: "Introduction & Overview"
        url: "/merit-badges/lifesaving/guide/"
  - group_title: "1. Safety Foundations"
    items:
      - title: "1. Safe Swim Defense"
        url: "/merit-badges/lifesaving/guide/req1/"
        is_sub: false
      - title: "2. Swimming Prerequisites"
        url: "/merit-badges/lifesaving/guide/req2/"
        is_sub: false
  - group_title: "3. Recognizing & Responding"
    items:
      - title: "a Drowning Situations"
        url: "/merit-badges/lifesaving/guide/req3a/"
        is_sub: true
      - title: "b Identifying Distress"
        url: "/merit-badges/lifesaving/guide/req3b/"
        is_sub: true
      - title: "c Rescue Order"
        url: "/merit-badges/lifesaving/guide/req3c/"
        is_sub: true
      - title: "d Adapting Rescue Techniques"
        url: "/merit-badges/lifesaving/guide/req3d/"
        is_sub: true
      - title: "e When NOT to Rescue"
        url: "/merit-badges/lifesaving/guide/req3e/"
        is_sub: true
  - group_title: "4. Reaching & Throwing Rescues"
    items:
      - title: "4. Reaching Rescues"
        url: "/merit-badges/lifesaving/guide/req4/"
        is_sub: false
      - title: "5. Throwing Rescues"
        url: "/merit-badges/lifesaving/guide/req5/"
        is_sub: false
  - group_title: "6. Rowing & Go Rescues"
    items:
      - title: "6. Rowing Rescue"
        url: "/merit-badges/lifesaving/guide/req6/"
        is_sub: false
      - title: "7. Go Rescue Aids"
        url: "/merit-badges/lifesaving/guide/req7/"
        is_sub: false
  - group_title: "8. Swimming Rescues (Conscious)"
    items:
      - title: "8. Buoyant Aid Rescues"
        url: "/merit-badges/lifesaving/guide/req8/"
        is_sub: false
      - title: "9. Nonbuoyant Aid Rescue"
        url: "/merit-badges/lifesaving/guide/req9/"
        is_sub: false
      - title: "10. Lead-and-Wait Techniques"
        url: "/merit-badges/lifesaving/guide/req10/"
        is_sub: false
      - title: "11. Contact Tows"
        url: "/merit-badges/lifesaving/guide/req11/"
        is_sub: false
      - title: "12. Escape Techniques"
        url: "/merit-badges/lifesaving/guide/req12/"
        is_sub: false
  - group_title: "13. Unconscious Victim Rescues"
    items:
      - title: "13. Unconscious Rescue"
        url: "/merit-badges/lifesaving/guide/req13/"
        is_sub: false
      - title: "14. Submerged Victim Recovery"
        url: "/merit-badges/lifesaving/guide/req14/"
        is_sub: false
  - group_title: "15. Spinal Injury & Resuscitation"
    items:
      - title: "15. Spinal Injury Management"
        url: "/merit-badges/lifesaving/guide/req15/"
        is_sub: false
      - title: "16. CPR & Rescue Breathing"
        url: "/merit-badges/lifesaving/guide/req16/"
        is_sub: false
      - title: "17. Other Injuries & Illnesses"
        url: "/merit-badges/lifesaving/guide/req17/"
        is_sub: false
  - group_title: "Beyond the Badge"
    items:
      - title: "Extended Learning"
        url: "/merit-badges/lifesaving/guide/extended-learning/"
---

Every year, thousands of people find themselves in trouble in the water — and someone nearby has the chance to save a life. The Lifesaving merit badge teaches you to be that someone. You will learn how to spot a person in distress, choose the safest rescue method, and act with confidence when every second counts.

Lifesaving is one of the most demanding and rewarding merit badges in all of Scouting. It is also one of the badges that can count toward earning the rank of Eagle Scout. The skills you build here are real — the kind that stay with you for the rest of your life.

## Then and Now

### Then — The Birth of Organized Rescue

For most of recorded history, drowning was simply accepted as a tragic part of life near water. That began to change in 1774, when a group of London doctors formed the **Royal Humane Society** — one of the first organizations dedicated to resuscitating drowning victims. Their early methods included rolling victims over barrels and blowing tobacco smoke into their lungs (not recommended today!).

By the late 1800s, surf lifesaving clubs were forming in Australia and the United States. Volunteer rescuers trained with ropes, boats, and cork life rings. The first official lifeguards appeared on American beaches in the early 1900s, and the American Red Cross began teaching water safety courses in 1914.

- **Purpose:** Save drowning victims using whatever was available
- **Mindset:** Brave volunteers rushing into the water — heroic, but often dangerous for the rescuer

### Now — Trained, Equipped, and Systematic

Modern lifesaving is built on science, training, and a clear order of rescue methods. Today's lifeguards and rescuers follow standardized protocols developed by the American Red Cross, the YMCA, and the United States Lifesaving Association. Equipment like rescue tubes, rescue boards, and automated external defibrillators (AEDs) are standard at pools and beaches across the country.

The biggest shift? The emphasis on **prevention**. Programs like the BSA's Safe Swim Defense and the CDC's drowning-prevention campaigns have dramatically reduced drowning rates. Modern lifesaving starts before anyone gets in the water.

- **Purpose:** Prevent drowning through education, supervision, and rapid response
- **Mindset:** Reach-Throw-Row-Go — always choose the safest rescue method first

---

**Get Ready!** You are about to learn skills that could genuinely save someone's life. This guide will prepare you for every requirement — from understanding water hazards to performing rescues and CPR. Take it seriously, practice with focus, and you will walk away from this badge a more capable and confident Scout.

{{< drg/image src="images/lifesaving-pool-training.avif" alt="Scouts in swimsuits practicing rescue techniques at a pool with a lifeguard instructor watching" >}}

---

## Kinds of Lifesaving

Water rescue happens in many different environments. Each one presents unique challenges. Understanding where rescues take place will help you adapt your skills to the real world.

### Pool Lifeguarding

Swimming pools are the most controlled aquatic environment. The water is clear, the depth is marked, and rescue equipment is close at hand. Most pool emergencies involve weak swimmers who drift into deep water, medical events like seizures, or unsupervised children.

{{< drg/safety-first >}}
Even in a pool, drowning can happen silently. A person in distress often cannot wave or shout — they are focused entirely on keeping their mouth above water. Constant vigilance is the lifeguard's most important tool.
{{< /drg/safety-first >}}

### Open-Water Lifesaving

Lakes, ponds, quarries, and reservoirs present challenges that pools do not. Water may be murky, the bottom uneven, and underwater hazards invisible. Temperature layers called **thermoclines** can cause sudden cold shock. There are no lane lines or walls to grab.

{{< drg/safety-first >}}
In open water, always swim with a buddy and stay within a designated swimming area. Cold water drains your strength much faster than warm water — even strong swimmers can become exhausted quickly.
{{< /drg/safety-first >}}

### Surf Rescue

Ocean beaches add waves, rip currents, and tidal changes to the mix. Surf rescuers use specialized equipment like rescue boards, fins, and personal watercraft. Reading the water — understanding where rip currents form and how waves break — is a critical skill.

{{< drg/safety-first >}}
Rip currents are the number one hazard at surf beaches. If caught in a rip current, do not fight it. Swim parallel to shore until you are out of the current, then swim back to the beach at an angle.
{{< /drg/safety-first >}}

{{< drg/image src="images/ocean-surf-rescue.avif" alt="A lifeguard running into ocean surf with a rescue buoy, waves breaking in the background" >}}

### Swiftwater Rescue

Rivers, streams, and floodwaters move — and moving water is incredibly powerful. Even knee-deep flowing water can knock an adult off their feet. Swiftwater rescue is a specialized discipline that uses throw bags, tethered swimmers, and technical rope systems.

{{< drg/safety-first >}}
Never enter moving water to attempt a rescue unless you have specific swiftwater training. The force of moving water is deceptive — a current moving at just 6 miles per hour exerts the same force per square foot as wind blowing at 270 mph.
{{< /drg/safety-first >}}

### Ice Rescue

Frozen lakes and ponds create a unique and extremely dangerous rescue environment. When ice breaks, victims plunge into water that is at or near freezing. Hypothermia sets in within minutes, and the ice around the break zone is often too weak to support a rescuer's weight.

{{< drg/safety-first >}}
If someone falls through ice, do NOT walk out onto the ice after them. Lie flat to distribute your weight and extend a pole, rope, or ladder. Call 911 immediately — ice rescue requires specialized training and equipment.
{{< /drg/safety-first >}}

{{< drg/did-you-know >}}
The United States Lifesaving Association estimates that lifeguards at guarded beaches rescue over 100,000 people from the surf every year. The survival rate for drowning at a beach staffed with trained lifeguards is over 99%.
{{< /drg/did-you-know >}}

{{< drg/image src="images/lake-lifeguard-stand.avif" alt="A lifeguard stand at a lake beach with a red rescue tube hanging on the side, swimmers visible in the background" >}}

---

Now let's dive into the requirements for the Lifesaving merit badge — starting with the foundation of every safe aquatic activity.

{{< drg/next-page
    text="You have learned about the history and types of lifesaving."
    teaser="Start with the principles of Safe Swim Defense — the foundation for everything that follows."
    url="/merit-badges/lifesaving/guide/req1/" >}}
