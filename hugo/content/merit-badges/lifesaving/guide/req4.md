---
title: "Req 4 — Reaching Rescues"
layout: guide
group_title: "Reaching & Throwing Rescues"
req_number: "4"
prev: "/merit-badges/lifesaving/guide/req3e/"
prev_title: "Req 3e — When NOT to Rescue"
next: "/merit-badges/lifesaving/guide/req5/"
next_title: "Req 5 — Throwing Rescues"
---

{{< drg/requirement number="4" >}}
Demonstrate "reaching" rescues using various items such as arm, leg, towels, shirts, paddles, poles.
{{< /drg/requirement >}}

Reaching rescues are the first method in the Reach-Throw-Row-Go sequence — and for good reason. They are the safest type of rescue because you never leave solid ground. Your feet stay planted on the pool deck, dock, or shoreline while you extend something for the victim to grab.

### The Golden Rule of Reaching

**Get low, stay anchored, and extend.** Every reaching rescue follows this pattern:

1. **Get low.** Lie flat on your stomach or drop to one knee. This lowers your center of gravity so you cannot be pulled in.
2. **Anchor yourself.** Grab something solid with your free hand — a dock post, a pool ladder, a tree root. If nothing is available, have a bystander hold your belt or ankles.
3. **Extend the reach.** Stretch your arm, leg, or object toward the victim.
4. **Pull slowly and steadily.** Once the victim has a grip, pull them in smoothly. Do not yank — you could lose your grip or pull the object away from them.

{{< drg/safety-first >}}
Never stand upright and lean out over the water. A panicking victim can generate a sudden, powerful tug that will pull you in headfirst. Always brace yourself in a stable, low position before extending your reach.
{{< /drg/safety-first >}}

### Reaching Tools — From Your Body to Your Environment

The beauty of a reaching rescue is that you can use almost anything. Here are the most common reaching aids, roughly in order from simplest to most effective:

**Your Arm or Leg:**
- Lie flat on the deck or dock and extend your arm toward the victim.
- If the victim is slightly farther, extend your leg — your reach is longer.
- Only use your body as a reaching tool when the victim is very close (within about 3 feet) and you are firmly anchored.

**A Towel or Shirt:**
- Roll or twist a towel or shirt into a rope-like shape.
- Hold one end firmly and extend or toss the other end to the victim.
- The victim grabs one end while you pull from the other.
- A wet towel is easier to throw accurately because the added weight keeps it straighter.

**A Paddle or Oar:**
- Extend the flat blade end toward the victim — it provides a larger surface to grab.
- Hold the handle end with both hands for a strong grip.
- Be careful not to hit the victim — extend slowly and let them grab on.

**A Pole or Branch:**
- Shepherd's crooks (the long poles with a hook at the end found at many pools) are ideal.
- A long tree branch works in natural settings.
- The longer the pole, the farther you can reach — but the harder it is to control.

{{< drg/tip >}}
Before your counselor session, look around the practice area and identify everything you could use for a reaching rescue. Pool noodles, kickboards, garden rakes, fishing rods, jump ropes — once you start looking, reaching aids are everywhere.
{{< /drg/tip >}}

{{< drg/image src="images/reaching-rescue-dock.avif" alt="A Scout lying flat on a dock, braced against a post, extending a long pole toward a swimmer in the water who is reaching for it" >}}

### Step-by-Step: Reaching Rescue with a Pole

Here is a detailed walkthrough of the most common reaching rescue:

1. **Spot the victim** and shout reassurance: "I see you — I'm going to help!"
2. **Get low** — lie flat on your stomach near the water's edge.
3. **Anchor** — grab a solid object with your non-dominant hand, or have someone hold your legs.
4. **Extend the pole** slowly toward the victim, keeping it just above the water surface.
5. **Guide the victim's hand** to the pole with your voice: "Grab the pole with both hands!"
6. **Wait for a firm grip** before you start pulling.
7. **Pull steadily** toward shore, keeping your body flat and your grip firm.
8. **Help them out** of the water once they reach the edge.

### Common Mistakes to Avoid

{{< drg/checklist title="Reaching Rescue Pitfalls" subtitle="What NOT to do" >}}
- Standing upright or kneeling at the very edge without bracing
- Extending your reach so far that you lose your balance
- Letting go of your anchor to get a few extra inches of reach
- Pulling too fast and ripping the aid out of the victim's hands
- Hitting the victim with the reaching aid — extend gently
- Forgetting to communicate — always talk to the victim
{{< /drg/checklist >}}

{{< drg/did-you-know >}}
The shepherd's crook — that long aluminum pole with a curved hook at the end — was invented specifically for pool rescues. The hook can be placed around a victim's body to pull them in even if they are unable to grab on.
{{< /drg/did-you-know >}}

{{< drg/external-link
    title="Red Cross: Reach or Throw, Don't Go"
    url="https://www.redcross.org/get-help/how-to-prepare-for-emergencies/types-of-emergencies/water-safety.html"
    description="Water safety tips including how to perform safe rescues from shore." >}}

{{< drg/next-page
    text="Reaching rescues keep you safe, but they have a limited range."
    teaser="When the victim is too far to reach, it's time to throw. Learn how to land a rescue device right where it's needed."
    url="/merit-badges/lifesaving/guide/req5/" >}}
